...
(function (global, factory) {
    ...
    // Native events
    const EVENT_TOUCH_END = IS_TOUCH_DEVICE
    ...
    const EVENT_POINTER_DOWN = HAS_POINTER_EVENT
        ? "pointerdown"
        : EVENT_TOUCH_START;
    const EVENT_POINTER_MOVE = HAS_POINTER_EVENT
        ? "pointermove"
        : EVENT_TOUCH_MOVE;
    ...
    function toCamelCase(value) {
        return value.replace(REGEXP_KEBAB_CASE, (substring) =>
    ...
    function emit(target, type, detail, options) {
        return target.dispatchEvent(
            new CustomEvent(
                type,
                Object.assign(
                    Object.assign(Object.assign({}, defaultEventOptions), {
                        detail,
                    }),
                    options,
                ),
    ...
    function getAdjustedSizes(data, type = SIZE_ADJUSTMENT_TYPE_CONTAIN) {
        ...
        if (isValidWidth && isValidHeight) {
            ...
            if (
                (type === SIZE_ADJUSTMENT_TYPE_CONTAIN &&
                    adjustedWidth > width) ||
    ...
    class CropperElement extends HTMLElement {
        ...
        constructor() {
            ...
            const name =
                (_b =
                    (_a = Object.getPrototypeOf(this)) === null || _a === void 0
                        ? void 0
        ...
        $getTagNameOf(name) {
            var _a;
            return (_a = tagNames.get(name)) !== null && _a !== void 0
                ? _a
                : name;
        }
        ...
        $addStyles(styles) {
            ...
            if (supportsAdoptedStyleSheets) {
                ...
                shadow.adoptedStyleSheets =
                    shadow.adoptedStyleSheets.concat(styleSheet);
            } else {
    ...
    class CropperCanvas extends CropperElement {
        ...
        $handlePointerDown(event) {
            ...
            if (
                ...
                (((type === "pointerdown" && event.pointerType === "mouse") ||
                    type === "mousedown") &&
                    ...
                    pointers2.forEach((pointer2) => {
                        ...
                        if (a2 > 0 || a1 > 0) {
                            ...
                            if (absRotateRate > maxRotateRate) {
                                ...
                                rotate = rotateRate;
                                centerX =
                                    (pointer.startX + pointer2.startX) / 2;
                                centerY =
                                    (pointer.startY + pointer2.startY) / 2;
                            }
                        }
                        x1 = Math.abs(x1);
                        y1 = Math.abs(y1);
        ...
        $toCanvas(options) {
            return new Promise((resolve, reject) => {
                if (!this.isConnected) {
                    reject(
                        new Error(
                            "The current element is not connected to the DOM.",
                        ),
                    );
                    return;
                }
                    ...
                    this.$actionStartTarget =
                        (_b =
                            (_a = event.detail) === null || _a === void 0
                                ? void 0
                                : _a.relatedEvent) === null || _b === void 0
                            ? void 0
                            : _b.target;
                };
            ...
            if (detail) {
                ...
                switch (action) {
                    case ACTION_MOVE:
                        if (this.translatable) {
                            ...
                            if (
                                ...
                                !(
                                    this.$actionStartTarget &&
                                    $selection.contains(this.$actionStartTarget)
                                )
                            ) {
                                this.$move(
                    ...
                    case ACTION_SCALE:
                        if (this.scalable) {
                            if (relatedEvent) {
                                const $selection = relatedEvent.target.closest(
                                    this.$getTagNameOf(CROPPER_SELECTION),
                                );
                                if (
                                    !$selection ||
                    ...
                    case ACTION_TRANSFORM:
                        if (this.rotatable && this.scalable) {
                            ...
                            if (relatedEvent) {
                                ...
                                const translateX =
                                    (moveX * d - c * moveY) / (a * d - c * b);
                                const translateY =
                                    (moveY * a - b * moveX) / (a * d - c * b);
                                /**
                                 * Equals to
    ...
    class CropperShade extends CropperElement {
        ...
        connectedCallback() {
            ...
            if ($canvas) {
                ...
                const $selection = $canvas.querySelector(
                    this.$getTagNameOf(CROPPER_SELECTION),
                );
                if ($selection) {
                    this.$onCanvasActionStart = (event) => {
                    ...
                    this.$onSelectionChange = (event) => {
                        const { x, y, width, height } = event.defaultPrevented
                            ? $selection
                            : event.detail;
    ...
    class CropperSelection extends CropperElement {
            ...
            switch (action) {
                ...
                case ACTION_MOVE:
                    if (
                        ...
                        (this.dynamic ||
                            (this.$actionStartTarget &&
                                this.contains(this.$actionStartTarget)))
                    ) {
                ...
                case ACTION_SCALE:
                    if (
                        relatedEvent &&
                        this.zoomable &&
                        (this.dynamic || this.contains(relatedEvent.target))
                    ) {
                        const offset = getOffset(currentTarget);
                        this.$zoom(
                            detail.scale,
        ...
        $toCanvas(options) {
            return new Promise((resolve, reject) => {
                ...
                const cropperImage = this.$canvas.querySelector(
                    this.$getTagNameOf(CROPPER_IMAGE),
                );
                if (!cropperImage) {
                    resolve(canvas);
                ...
                cropperImage
                    ...
                    .then((image) => {
                        ...
                        if (context) {
                            ...
                            const translateX =
                                (offsetX * d - c * offsetY) / (a * d - c * b);
                            const translateY =
                                (offsetY * a - b * offsetX) / (a * d - c * b);
    ...
    class CropperViewer extends CropperElement {
        ...
        connectedCallback() {
            ...
            if (this.selection) {
                $selection =
                    (_b =
                        (_a = getRootDocument(this)) === null || _a === void 0
                            ? void 0
                            : _a.querySelector(this.selection)) !== null &&
                    _b !== void 0
                        ? _b
                        : null;
            } else {
                $selection = this.closest(
                    this.$getTagNameOf(CROPPER_SELECTION),
                );
            }
            if (isElement($selection)) {
                ...
                const $canvas = $selection.closest(
                    this.$getTagNameOf(CROPPER_CANVAS),
                );
                if ($canvas) {
                    this.$canvas = $canvas;
                    const $sourceImage = $canvas.querySelector(
                        this.$getTagNameOf(CROPPER_IMAGE),
                    );
                    if ($sourceImage) {
                        ...
                        this.$onSourceImageLoad =
                            this.$handleSourceImageLoad.bind(this);
                        this.$onSourceImageTransform =
                            this.$handleSourceImageTransform.bind(this);
                        on(
                            $sourceImage.$image,
                            EVENT_LOAD,
                            this.$onSourceImageLoad,
        ...
        $handleSelectionChange(event) {
            this.$render(
                event.defaultPrevented ? this.$selection : event.detail,
            );
        }
            ...
            if (this.$sourceImage) {
                ...
                setTimeout(() => {
                    this.$transformImageByOffset(
                        matrix !== null && matrix !== void 0
                            ? matrix
                            : this.$sourceImage.$getTransform(),
                        -x,
                        -y,
                    );
                });
    ...
    class Cropper {
        constructor(element, options) {
            ...
            if (container) {
                if (isString(container)) {
                    container =
                        (_a = getRootDocument(element)) === null ||
                        _a === void 0
                            ? void 0
                            : _a.querySelector(container);
                }
            ...
            if (template && isString(template)) {
                ...
                Array.from(
                    documentFragment.querySelectorAll(CROPPER_IMAGE),
                ).forEach((image) => {
                    image.setAttribute("src", src);
                    image.setAttribute(
                        "alt",
                        element.alt || "The image to crop",
                    );
        ...
        destroy() {
            ...
            if (cropperCanvas) {
                (_a = cropperCanvas.parentElement) === null || _a === void 0
                    ? void 0
                    : _a.removeChild(cropperCanvas);
            }
            if (this.element) {
