...
function modifyServicesWithTimesNewReservByIndex(index) {
    ...
    if (!horaFinElem || !horaInicioElem || !servicioElem || !empleadoElem) {
        console.warn(
            "Algunos elementos necesarios no se encontraron en el DOM.",
        );
        return;
    }
    ...
    let idServicio = parseInt(
        servicioElem.getAttribute("data-service").trim(),
        10,
    );
    let idEmpleado = empleadoElem.getAttribute("data-empleid").trim();
    let seleccionaClienteValor = obtenerValorCorazon(
        ".solicitadoClientePantalla2",
    );
...
function modifyServicesWithTimesByIndex(index) {
    let horaFin = document
        .querySelector(".slotHoraFinCorbrarServicioAdd")
        .textContent.trim();
    let horaInicio = document
        .querySelector(".slotHorasCobrarServicioAdd")
        .textContent.trim();
    let duracion = calcularDuracion(horaInicio, horaFin);
    console.log(duracion, horaInicio, horaFin, index, "DURACION");

    let idServivioAddTarjetaBlanca = document
        .querySelector(".selectServiceAdd")
        .getAttribute("data-service");
    let idEmpleado = document
        .querySelector(".slotEmpleadoAdd")
        .getAttribute("data-empleid")
        .trim();
    let serleccionaClienteValor = obtenerValorCorazon(
        ".solicitadoClientePantallaInfoClientePantallaDos",
    );
...
function addServiceDiscountArray(index) {
    ...
    if (importeDescuentoServi) {
        ...
        if (descuentosVentaRapida[index]) {
            ...
            descuentosVentaRapida[index].descuentoServicio = parseFloat(
                importeDescuentoServi,
            );
        } else {
...
function actualizarDescuentoPrecioArrayVentaRapida(index) {
    ...
    let imputDescuentoPorcentaje =
        document.getElementById("uid-319-input").value;
    ...
    for (let i = 0; i < serviciosVentaRapida.length; i++) {
        if (i === indexActualizar) {
            ...
            serviciosVentaRapida[i].descuento_servicio = parseInt(
                imputDescuentoPorcentaje,
            );
            break;
        }
    }
...
function insertarServicioEmpleadoArrayVentaRapida(
    id_servicio,
    nombreEmpleado,
    precioServicio,
    idEmpleado,
...
let eliminarNotificacionReserva = document.getElementById(
    "remove_notification_reserv",
);
    ...
    calendar = new FullCalendar.Calendar(calendarEl, {
        ...
        viewDidMount: function (info) {
            ...
            $(".fc-day-sun .fc-daygrid-day-events").each(function () {
                if ($(this).find("span").length === 0) {
                    $(this).append(
                        '<span style="color: #8c8b88;text-transform: none;font-size: .75rem;font-weight: 500;line-height: 1.5em;">Día libre</span>',
                    );
                }
            });
        ...
        eventDidMount: function (info) {
            ...
            if (evento.extendedProps) {
                const tieneNota =
                    evento.extendedProps.nota ||
                    evento.extendedProps.nota_interna;
                const clienteSeleccionado =
                    evento.extendedProps.seleccionado_cliente === 1;
                const estaPagado =
                    evento.extendedProps.status_payment === "Pagado";
                const tieneAdvertencia =
                    evento.extendedProps.confirma_cliente_modificacion ===
                    "pendiente";
                ...
                if (tieneNota && clienteSeleccionado && estaPagado) {
                    agregarAdvertencia();
                    const combinado = createDivWithNoteCora(
                        document.createElement("div"),
                    ); // esto incluye nota + cora
                ...
                if (eventEl) {
                    ...
                    eventEl.setAttribute(
                        "data-idreserv",
                        evento.extendedProps.reservaId,
                    );
                }
                if (
                    evento.extendedProps &&
                    evento.extendedProps.empleada &&
                    evento.extendedProps.empleada.imagenEmple
                ) {
                    eventEl.setAttribute(
                        "data-urlImgEmple",
                        evento.extendedProps.empleada.imagenEmple,
                ...
                document
                    ...
                    .setAttribute("data-date", evento.extendedProps.fecha);
                if (eventEl) {
                    if (evento.extendedProps.servicio) {
                        eventEl.style.setProperty(
                            "background-color",
                            lightenedColor(
                                evento.extendedProps.servicio.borderColor,
                                70,
                                0.52,
                            ),
                        );
                        eventEl.style.setProperty(
                            "border-left",
                            `4px solid ${evento.extendedProps.servicio.borderColor}`,
                            "important",
                        );
                        eventEl.style.setProperty(
                            "border-top",
                            `1px solid ${evento.extendedProps.servicio.borderColor}`,
                            "important",
                        );
                    }
                }
                ...
                let despleStatus = document.querySelector(
                    ".header_actions_oRFfx",
                );
                if (evento.extendedProps.status === "Finalizada") {
                    ...
                    evento.extendedProps.status === "cancelled" ||
                ...
                if (
                    new Date(evento.end) < now &&
                    evento.extendedProps.status !== "completada" &&
                    evento.extendedProps.status !== "cancelled" &&
                    evento.extendedProps.status !== "pagada" &&
                    evento.extendedProps.status !== "Finalizada" &&
        ...
        eventDrop: function (info) {
            ...
            if (
                ...
                parseInt(nuevoEmpleadoId) === parseInt(empleadoAnteriorId)
            ) {
                console.log(
                    "El evento no ha cambiado. No se realiza actualización.",
                );
                info.revert();
                return;
            }
                                ...
                                success: function (data) {
                                    if (data.empleado) {
                                        evento.setExtendedProp("empleada", {
                                            ...
                                            apellido:
                                                data.empleado.primerApellido,
                                            imagenEmple:
                                                data.empleado.img_empleada,
        ...
        eventClick: function (info) {
            ...
            if (document.querySelectorAll("div.mensajeResolucion")) {
                document
                    .querySelectorAll("div.mensajeResolucion")
            ...
            if (info.event.extendedProps.multiple === null) {
                ...
                colorBordeNewReservCalendar =
                    info.event.extendedProps.servicio.borderColor;
                colorBordeSuperior =
                    info.event.extendedProps.servicio.borderColor;
                ...
                let serviciosMultiples = events.filter(
                    (event) => event.extendedProps.multiple === multiple_id,
                );
                ...
                serviciosMultiples.forEach((event) => {
                    ...
                    colorBordeReservArray.push(
                        event.extendedProps.servicio.borderColor,
                    );
                });
...
function startCalendarAutoRefresh() {
    ...
    intervalRefetch = setInterval(() => {
        const isOffcanvasOpen =
            ...
            document
                .getElementById("offcanvasShowAllServicesChange")
                .classList.contains("show") ||
            document
                .getElementById("offcanvasAddServicesChange")
                .classList.contains("show") ||
            document
                .getElementById("offcanvasSelectClient")
                .classList.contains("show") ||
            document
                .getElementById("offcanvasShowAllServicesChangeAdd")
                .classList.contains("show") ||
            document
                .getElementById("newReservCalendar")
                .classList.contains("show");
...
function updateTableView(viewType) {
    ...
    const tbody = document.querySelector(
        ".fc-scrollgrid-section-body table tbody",
    );
...
function showHideEmpleDia(idEmpleado) {
    ...
    if (vistaDia) {
        ...
        const tds = vistaDia.querySelectorAll(
            `td[data-resource-id="${idEmpleado}"]`,
        );
        const th = vistaDia.querySelector(
            `th[data-resource-id="${idEmpleado}"]`,
        );
        if (th) {
...
function showHideEmpleSemana(idEmpleado) {
    ...
    const ths = vistaDia.querySelectorAll(
        `th[data-resource-id="${idEmpleado}"]`,
    );
    ...
    ths.forEach(function (th) {
        if (th) {
            ...
            if (th.classList.contains("thOculto")) {
                ...
                const cushion = th.querySelector(
...
var linksMasDos = document.querySelectorAll(
...
function montarOffcanvasLateralTodaInfo(info, id_multipleCalendar = null) {
    ...
    if (id_multipleCalendar === null) {
        ...
        if (extendedProps.mensaje_cliente !== null) {
            ...
            indicadorNota.classList.contains("b-tabs_tabIndicator_vu4Y2")
                ? null
                : indicadorNota.classList.add("b-tabs_tabIndicator_vu4Y2");
        ...
        let corazon = document.querySelector(
            ".solicitadoClientePantallaInfoCliente",
        );
        if (evento.extendedProps.seleccionado_cliente === 1) {
            corazon.src =
                urlAplicacion + "/storage/calendar/corazonRojoEmpleCliente.svg";
        ...
        if (existeCliente) {
            ...
            insertarTarjetaClienteSelecionadoExtentrop(
                usuario,
                ".basket-customer-card0101Info",
                "card_empty_info",
            );
        } else {
            showDivClienInfo("basket-customer-card0101Info");
        ...
        let divTarjetasIniciales = document.querySelector(
            ".subbookings-list_container_nMAxs .tarjetasIncialesMostrarOcultar",
        );
        divTarjetasIniciales.classList.remove("d-none");

        //vaciamos div reserva multiple
        let divTarjetasNuevas = document.querySelector(
            ".subbookings-list_container_nMAxs .nuevasTarjetasMostrarOcultar",
        );
        ...
        if (extendedProps.status_payment === "Pagado") {
            ...
            let fechaReservaPagada2 =
                formatearFechaSiReservaPagada(fechaReservaPagada);
            let pagoTotal = comprobarSiPagoFraccionadoObtenerTotal(
                extendedProps.payments,
            );
        ...
        setClass(
            ...
            ".headerInfoService",
            extendedProps.confirma_cliente_modificacion,
        );
        ...
        meterHorasArrayInicio(
            ".slotHorasCobrarServicio",
            ".slotHoraFinCorbrarServicio",
            ".slotEmpleadoAddInicio",
            extendedProps.seleccionado_cliente,
            extendedProps.duracion,
            idUnico,
        );
        ...
        if (idOffcanvasBig) {
            idOffcanvasBig.setAttribute(
                "data-idReserv",
                extendedProps.reservaId,
            );
        }
        ...
        document.querySelector(
            ".services_serviceDecorator_ldMxA",
        ).style.borderColor = `${extendedProps.servicio.borderColor}`;
        document
            .querySelector(".services_serviceWrapper_gug5x")
            .setAttribute("data-idServiceAdd", extendedProps.servicio.id);
        document
            .querySelector(".services_serviceWrapper_gug5x")
            .setAttribute("data-id-unico", idUnico);
        document
            .querySelector(".services_serviceWrapper_gug5x")
            .setAttribute("data-id-reserv", extendedProps.reservaId);
        ...
        document
            .querySelector(".services_serviceDuration_Zb36z")
            .setAttribute("data-oldDuration", extendedProps.servicio.duracion);
        document.querySelector(".services_servicePrice_wErzf").textContent =
            extendedProps.servicio.precio;
        document.querySelector(".slotHorasCobrarServicio").textContent =
            formatTime(extendedProps.fecha);
        document
            .querySelector(".slotHorasCobrarServicio")
            .setAttribute("data-hourReserv", formatTime(extendedProps.fecha));
        document.querySelector(".slotHoraFinCorbrarServicio").textContent =
            obtenerHoraEuropaCentral(info.event.end);
        ...
        const contenedor = document.querySelector(
            ".contenedorHorasFin .scrollable",
        );
        const contenedor2 = document.querySelector(
            ".contenedorHorasInicio .scrollable",
        );
        ...
        const eventDetailsModal = new bootstrap.Offcanvas(
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (serviciosMultiple.length > 0) {
                    ...
                    let htmlContent = "";
                    let totalPricePay = 0;
                    let fechaReserva = formatDate2(
                        serviciosMultiple[0].date_time,
                    );
                    document.querySelector(".fechaCitaInfo").textContent =
                        fechaReserva;

                    // Crear un array con solo los service_id usando map()
                    let serviceIds = serviciosMultiple.map(
                    ...
                    getServicesById(serviceIds, function (servicios) {
                        getAllEmpleados(function (empleadosReservas) {
                            ...
                            serviciosMultiple.forEach((service) => {
                                ...
                                let empleado = empleadosReservas.find(
                                    (emp) =>
                                        emp.id ===
                                        parseInt(service.empleada_id),
                                );
                                if (empleado) {
                                    // Obtenemos las primeras dos letras del nombre del empleado
                                    let iniciales = empleado.nombre
                                        .substring(0, 2)
                                        .toUpperCase();
                            ...
                            arrayIdsReservas = infoArrayEnvio.map(
                                (evento) => evento.extendedProps.reservaId,
                            );
                            // console.log(arrayIdsReservas,serviciosMultiple, "SERVICIOSMULTIPLES IDS");
                            let idsEventosParaHtml = infoArrayEnvio.map(
                                (e) => e.id,
                            );
                            ...
                            servicios.forEach((servicio, index) => {
                                let fecha =
                                    serviciosMultiple[index].date_time.split(
                                        " ",
                                    )[0];
                                let horaInicio03 = formatTime(
                                    serviciosMultiple[index].date_time,
                                );
                                let horaFin03 = calculateEndTime(
                                    serviciosMultiple[index].date_time,
                                    serviciosMultiple[index].duration,
                                );
                                let id_empleada03 =
                                    serviciosMultiple[index].empleada_id;
                                let seleccionaCliente03 =
                                    serviciosMultiple[index]
                                        .empleado_seleccionado;
                                let duracion03 =
                                    serviciosMultiple[index].duration;

                                //en la función meterHOras... se añade el id_empleado al id_unico
                                meterHorasArrayMultiple(
                                    servicio.id,
                                    horaInicio03,
                                    horaFin03,
                                    id_empleada03,
                                ...
                                let tiempoFormateada = comprobar603090(
                                    serviciosMultiple[index].duration,
                                );
                                ...
                                let nombreEmpleado55 =
                                    nombreEmpleadosArray[index];
                                let inicialesEmpleado33 =
                                    inicialesEmpleados[index];
                                let horaInicio = formatTime(
                                    serviciosMultiple[index].date_time,
                                );
                                let horaFin = calculateEndTime(
                                    serviciosMultiple[index].date_time,
                                    serviciosMultiple[index].duration,
                                );
                            ...
                            addHtmlDivPrecioFinal(
                                totalPricePay,
                                '#eventDetailsModal p[data-testid="appointment-price"]',
                                '#eventDetailsModal div[data-testid="appointment-to-be-paid"]',
                            );
                            let reservaTerminaMasTarde =
                                getReservaMasTarde(serviciosMultiple);
                            if (divCombo && despleStatus) {
                                divCombo.classList.remove("d-none"); // Mostrarlo de forma predeterminada
                                // Limpiar el desplegable de acciones
                                $(despleStatus).empty();
                            ...
                            setClass(
                                reservaTerminaMasTarde.status,
                                ".headerInfoService",
                                info[0].extendedProps
                                    .confirma_cliente_modificacion,
                            );
                            let eventDetailsModal = new bootstrap.Offcanvas(
                                document.getElementById("eventDetailsModal"),
                            );
                            eventDetailsModal.show();
                            ...
                            setTimeout(() => {
                                try {
                                    eventIdChangeCalendarArray.forEach(
                                        (eventId, index) => {
                                            ...
                                            if (!eventInfo) {
                                                console.warn(
                                                    "⚠️ Evento no encontrado en el calendario:",
                                                    eventId,
                                                );
                                                return;
                                            }

                                            let fechaFlatpickrDiv =
                                                document.querySelector(
                                                    ".fechaCitaInfo",
                                                );
                                            ...
                                            let fechaFlatpickr =
                                                fechaFlatpickrDiv.getAttribute(
                                                    "data-datePiker",
                                                );
                                            if (!fechaFlatpickr) return;

                                            eventInfo.setStart(fechaFlatpickr);
                                            eventInfo.setProp("classNames", [
                                                "temporal",
                                            ]);
                                            eventInfo.setProp(
                                                "borderColor",
                                    ...
                                    console.error(
                                        "Error controlado en modificación múltiple:",
                                        e,
                                    );
                            ...
                            let tarjetasServiciosMultiples =
...
function obtenerIdUnicoSinEvent(data) {
    ...
    if (data.extendedProps.servicio) {
        ...
        let tarjetaServicio = document.querySelector(
            ".services_serviceInfo_iDMQwAddCalendar",
        );
        if (tarjetaServicio) {
...
function prepararVistaParaReservaPagadaEnOffcanvas() {
    ...
    divApagarOcultar
        ? divApagarOcultar.classList.add("d-none")
        : divApagarOcultar.classList.add("");
    divTotalAPagar
        ? divTotalAPagar.classList.remove("b-border-r")
        : divTotalAPagar.classList.remove("");
    botonCobrarOcultar
        ? botonCobrarOcultar.classList.add("d-none")
        : botonCobrarOcultar.classList.add("");
...
function quitarVistaReservaPagada() {
    ...
    divApagarOcultar
        ? divApagarOcultar.classList.remove("d-none")
        : divApagarOcultar.classList.remove("");
    divTotalAPagar
        ? divTotalAPagar.classList.add("b-border-r")
        : divTotalAPagar.classList.add("");
    botonCobrarOcultar
        ? botonCobrarOcultar.classList.remove("d-none")
        : botonCobrarOcultar.classList.remove("");
...
function cambiarTotales(total) {
    ...
    const visualizaPrecio = document.querySelector(
        '[data-testid="appointment-price"]',
    );
    if (visualizaPrecio) {
        visualizaPrecio.textContent = total;
    }
    const DivvisualizaPrecioAPagar = document.querySelector(
        '[data-testid="appointment-to-be-paid"]',
    );
    if (DivvisualizaPrecioAPagar) {
        let visualizadorPagarP =
            DivvisualizaPrecioAPagar.querySelector(".b-heading-xl");
        visualizadorPagarP.textContent = total;
    }
    const visualizaPrecio2 = document.querySelector(
        '[data-testid="appointment-price2"]',
    );
    if (visualizaPrecio2) {
        visualizaPrecio2.textContent = total;
    }
    const DivvisualizaPrecioAPagar2 = document.querySelector(
        '[data-testid="appointment-to-be-paid2"]',
    );
    if (DivvisualizaPrecioAPagar2) {
        let visualizadorPagarP2 =
            DivvisualizaPrecioAPagar2.querySelector(".b-heading-xl");
...
function initDatePiker() {
    ...
    flatpickr(fechaCitaInfo, {
        ...
        onChange: function (selectedDates, dateStr, instance) {
            ...
            document
                .querySelector(".fechaCitaInfo")
                .setAttribute("data-datepiker", selectedDates);
            ...
            let existenTarjetasDrag = document.querySelectorAll(
                ".nuevasTarjetasMostrarOcultar .subboking-drag-el",
            );
            if (existenTarjetasDrag.length > 0) {
                ...
                setTimeout(() => {
                    ...
                    cambiarFechaEvento(
                        eventIdChangeCalendar.trim(),
                        start,
                        end,
                    );
                    document
                        .querySelector(".fechaCitaInfo")
                        .setAttribute("data-date", dateStr);
                }, 400);
            }
...
function mostrarEventosArrayMejorado(datepiker) {
    ...
    getServicesById(selectedServiceIds2, function (servicios) {
        ...
        if (document.getElementById(datepiker)) {
            fecha55 = document
                .getElementById(datepiker)
                .getAttribute("data-datepiker");
        } else {
            fecha55 = document
                .querySelector(datepiker)
                .getAttribute("data-datepiker");
        }
        if (
            existeNombreClienteComun(".basket-customer-card0101Info") !== false
        ) {
        ...
        setTimeout(() => {
            servicios.forEach((servicio, index) => {
                let eventoTemporal = document.querySelector(
                    `.fc-event.temporalArray_${index}`,
                );
                if (eventoTemporal) {
                    ...
                    // }
                    eventoTemporal.setAttribute(
                        "data-idunicoevento",
                        servicesWithTimes[index].id_unico,
                    );
                    eventoTemporal.style.setProperty(
                        "border-left",
                        `4px solid ${servicio.borderColor}`,
                        "important",
                    );
                    eventoTemporal.style.setProperty(
                        "border-top",
                        `1px solid ${servicio.borderColor}`,
                        "important",
                    );
                }
            });
...
function initDatePikerClient() {
    ...
    if (calendarFechaCliente) {
        flatpickr(calendarFechaCliente, {
            ...
            onChange: function (selectedDates, dateStr, instance) {
                calendarFechaCliente.setAttribute(
                    "data-happy-date",
                    convertir_string_dataDate(dateStr),
                );
...
function closedOffcanvasInfoReserv() {
    ...
    insertartarjetaSeleccionaCliente(
        ".basket-customer-card0101Info",
        "card_empty_info",
    );
    ...
    if (pestaniaCita) {
        document
            .querySelectorAll(".b-tabs_tabDefaultActive_CYkQd")
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const headersAdd = document.querySelectorAll(
        ".services-list_header_zR0q6Add",
    );
...
const tarjetaSecundaria = document.querySelector(
    ".services_serviceInfo_iDMQwAdd",
);
if (tarjetaSecundaria) {
    tarjetaSecundaria.addEventListener("click", function () {
        toggleOffcanvas(
            "offcanvasShowAllServicesChangeAdd",
            "offcanvasAddServicesChange",
        );
    });
}

//FUNCIÓN PARA METER TARJETA de servicio en vista modificar servicio
function aniadirServicioHtml(
    colorBorde,
    nombre,
    duracion,
    precio,
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const targetServicesAdd = document.querySelectorAll(
        ".services-list_serviceVariant_i9qZrAdd",
    );
        ...
        targetServicesAdd.forEach((targetAdd) => {
            targetAdd.addEventListener("click", function () {
                if (botonEditarServicioReserva) {
                    let newReserv =
                        botonEditarServicioReserva.getAttribute("data-new");
                    console.log(
                        newReserv,
                ...
                let id_serviceChangeAdd = targetAdd.getAttribute(
                    "data-serviciochange",
                ); //id_servicio seleccionado
                var csrfToken = $('meta[name="csrf-token"]').attr("content");
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if (data.encontrado) {
                            ...
                            aniadirServicioHtml(
                                data.servicio.borderColor,
                                data.servicio.nombre,
                                data.servicio.duration,
                            ...
                            document
                                .querySelector(".selectServiceAdd")
                                .setAttribute("data-service", data.servicio.id);

                            //cambiamos hora fin poniendo la horainicio + lo que dura el servicio
                            let horaInicioAdd2 = document.querySelector(
                                ".slotHorasCobrarServicioAdd",
                            ).textContent;
                            let divContenedorHoraFinAdd =
                                document.querySelector(
                                    ".slotHoraFinCorbrarServicioAdd",
                                );
                            let duracionCompleta =
                                data.servicio.duration.toString() + "min";
                            divContenedorHoraFinAdd.textContent =
                                calcularHoraFin(
                                    horaInicioAdd2,
                                    duracionCompleta,
                                );
                            let horaFinAdd = calcularHoraFin(
                                horaInicioAdd2,
                                duracionCompleta,
                            );
                            marcarHoraSeleccionada(
                            ...
                            // Hacer scroll a la hora activa en ambos contenedores
                            const contenedor = document.querySelector(
                                ".contenedorHorasFinAdd .scrollable",
                            );
                            //función para hacer scroll hora inicio y fin activas
                            function intentarScroll(contenedor) {
                                if (!contenedor) return;
                            ...
                            let botones = [
                                ...
                                document.getElementById(
                                    "uid-377-inputEditReserv",
                                ),
                            ];

                            // ¿Hay algún empleado seleccionado?
                            let empleadoSeleccionado = textosEmpleado.some(
                                (selector) => {
                            ...
                            let eventoTemporalCalendario2 =
                                document.querySelector(".fc-event.temporal2");
    ...
    const targetServices = document.querySelectorAll(
        ".services-list_serviceVariant_i9qZr",
    );
    if (targetServices) {
        targetServices.forEach((target) => {
            target.addEventListener("click", function (event) {
                ...
                let id_serviceChange = target.getAttribute(
                    "data-serviciochange",
                );
                var csrfToken = $('meta[name="csrf-token"]').attr("content");
                var url = "get-serviceById";
                    ...
                    success: function (data) {
                        if (data.encontrado) {
                            ...
                            let serviceName = document.querySelector(
                                ".services_serviceName_YhbTW",
                            );
                            let servicePrice = document.querySelector(
                                ".services_servicePrice_wErzf",
                            );
                            ...
                            let eventoTemporalCalendario =
                                document.querySelector(".fc-event.temporal");
                            if (eventoTemporalCalendario) {
                                ...
                                showDivBotonGuardarInfo(
                                    "saveChangesFooterInfo",
                                );
                                colorBordeNewReservCalendar =
                                    data.servicio.borderColor;
                                colorBordeSuperior = data.servicio.borderColor;
                            ...
                            let eventDetailsOffcanvas =
                                bootstrap.Offcanvas.getInstance(
                                    document.getElementById(
                                        "offcanvasShowAllServicesChange",
                                    ),
                                );
...
function obtenerHoraInicioInicio() {
    ...
    if (divHoraInicioIcinial2) {
        let horaIncioIncio = document.querySelector(
            ".slotHorasCobrarServicio",
        ).textContent;
...
function obtenerHoraFinInicio() {
    if (document.querySelector(".slotHoraFinCorbrarServicio")) {
        let horaFinInicio = document.querySelector(
            ".slotHoraFinCorbrarServicio",
        ).textContent;
...
function obtenerIdEmpleInicio(slotEmpleado) {
    ...
    if (document.querySelector(slotEmpleado)) {
        return document
            .querySelector(slotEmpleado)
            .getAttribute("data-empleado");
    }
}
...
function meterHorasArrayInicio(
    slotHoraIncio,
    slotHoraFin,
    slotEmpleadoIncio,
    seleccionaCliente_value,
    duracion,
    idUnico = null,
) {
    //obtener hora inicio fin y id servicio pantalla principal
    setTimeout(() => {
        let horaIncioPantallaPrincipalNoModify =
            document.querySelector(slotHoraIncio).textContent;
        let horaFinPantallaPrincipalNoModify =
...
function meterHorasArrayInicioNoTimeOut(
    slotHoraIncio,
    slotHoraFin,
    slotEmpleadoIncio,
    seleccionaCliente_value,
    ...
    let horaFinPantallaPrincipalNoModify =
        document.querySelector(slotHoraFin).textContent;
    let durationCalendar = calcularDuracion(
        horaIncioPantallaPrincipalNoModify,
        horaFinPantallaPrincipalNoModify,
    );
...
function meterHorasArrayMultiple(
    serviceId,
    horaIncio,
    horaFin,
    idEmpleada,
    ...
    let id_unico =
        ...
        serviceId +
        "_" +
        horaIncio.replace(/[\s:-]/g, "_") +
        "__" +
        horaFin.replace(/[\s:-]/g, "_") +
        "_" +
        idEmpleada;
    servicesWithTimes.push({
        id_unico: id_unico,
...
function meterHorasArraySecundaria(
    slothoraInicio,
    slotHoraFin,
    slotEmpleado,
    duracion,
    ...
    let horaIncioScreenAddService = document
        .querySelector(slothoraInicio)
        .textContent.trim();
    let seleccionaCliente = document.getElementById("solicictaCliente").value;

    let id_reserva = null;

    let horaFinScreenAddService = document
        .querySelector(slotHoraFin)
        .textContent.trim();
    ...
    selectedServiceIds.forEach((serviceId, index) => {
        ...
        servicesWithTimes.push({
            id_unico:
                ...
                serviceId +
                "_" +
                horaIncioScreenAddService.replace(/[\s:-]/g, "_") +
                "__" +
                horaFinScreenAddService.replace(/[\s:-]/g, "_") +
                "_" +
                idEmpleadaSecun,
            id: parseInt(serviceId, 10),
...
function formatearDuracion(servicio_id) {
    ...
    const serviceTime = servicesWithTimes.find(
        (time) => time.id === servicio_id,
    );
...
function construirHtmlTarjetasFinales(
    borderColor,
    nombre,
    precio,
    duracionServicio,
    ...
    if (id_unicoEnviado === null) {
        id_unicoFinal =
            fechaEventoIdUnico + "_" + id_servicio + "_" + id_unico1;
    } else {
...
function showModifyService(botonServiceModify) {
    ...
    document
        .getElementById("uid-377-input")
        .classList.remove("index_is--disabled_w97Nq");
    document
        .getElementById("uid-377-inputEditReserv")
        .classList.remove("index_is--disabled_w97Nq");
    ...
    if (newReserv === true || newReserv === "true") {
        ...
        document
            .getElementById("uid-377-inputCalendar")
            .classList.remove("index_is--disabled_w97Nq");

        if (
            document
                .querySelector(".allServicesAddCalendar00")
                .classList.contains("allservicesModificarInicialCalendar")
        ) {
            document
                .querySelector(".allServicesAddCalendar00")
                .classList.remove("allservicesModificarInicialCalendar");
        } else if (
            document
                .querySelector(".allServicesAddCalendar00")
                .classList.contains("allservicesVistaOtroServicioCalendar")
        ) {
            document
                .querySelector(".allServicesAddCalendar00")
                .classList.remove("allservicesVistaOtroServicioCalendar");
        }
        document.querySelector(".newReservCalendar00").style.display = "none";
        document.querySelector(".allServicesAddCalendar00").style.display =
            "none";
        document.querySelector(".allServicesAddCalendar00Add").style.display =
            "block";

        document.querySelector(
            ".buttonEditStrackModifyNewReserv",
        ).style.display = "flex";
        document.querySelector(
            ".buttonSavetrackModifyNewReserv",
        ).style.display = "none";
    } else {
    ...
    //ponemos el título
    let posicion = infoArrayEnvio.findIndex(
        (evento) => evento.id === id_evento33,
    );

    let divTitulo = "";
    if (newReserv === true || newReserv === "true") {
        divTitulo = document.querySelector(
            ".allServicesAddCalendar00Add .b-custom-header_headerTitle_ogW55",
        );
    } else {
        divTitulo = document.querySelector(
            "#offcanvasAddServicesChange .b-custom-header_headerTitle_ogW55",
        );
    }
    let titulo = `
    ...
    if (event.extendedProps.servicio) {
        ...
        if (newReserv === true || newReserv === "true") {
            ...
            if (duracion026) {
                ...
                duracion026 = calcularDuracion(
                    obtenerHoraEuropaCentral(event.start),
                    obtenerHoraEuropaCentral(event.end),
                );
            }
            aniadirServicioHtml(
                event.extendedProps.servicio.borderColor,
                event.extendedProps.servicio.nombre,
                duracion026,
                event.extendedProps.servicio.precio,
                ".selectServiceAddCalendar",
                "services_serviceInfo_iDMQwAddCalendar",
            );
            document
                .querySelector(".selectServiceAddCalendar")
                .setAttribute("data-service", event.extendedProps.servicio.id);
        } else {
            aniadirServicioHtml(
                event.extendedProps.servicio.borderColor,
                event.extendedProps.servicio.nombre,
                event.extendedProps.duracion,
                event.extendedProps.servicio.precio,
                ".selectServiceAdd",
                "services_serviceInfo_iDMQwAdd",
            );
    ...
    if (newReserv === true || newReserv === "true") {
        ...
        document.querySelector(
            ".slotHorasCobrarServicioAddCalendar",
        ).textContent = obtenerHoraEuropaCentral(event.start);
        document.querySelector(
            ".slotHoraFinCorbrarServicioAddCalendar",
        ).textContent = obtenerHoraEuropaCentral(event.end);
        ...
        const contenedor2 = document.querySelector(
            ".contenedorHorasInicioAdd .scrollable",
        );
        const contenedor35 = document.querySelector(
            ".contenedorHorasInicioAddCalendar .scrollable",
        );
        const contenedor45 = document.querySelector(
            ".contenedorHorasFinAddCalendarclass  .scrollable",
        );
        const contenedor65 = document.querySelector(
            ".contenedorHorasFinAddCalendar .scrollable",
        );
        ...
        document.querySelector(
            ".slotEmpleadoAddInicioCalendarAdd",
        ).textContent = event.extendedProps.empleada.nombre;
        document
            .querySelector(".slotEmpleadoAddInicioCalendarAdd")
            .setAttribute("data-empleid", event.extendedProps.empleada.id);
        document
            .querySelector(".slotEmpleadoAddInicioCalendarAdd")
            .setAttribute("data-empleado", event.extendedProps.empleada.id);
        ...
        //hora inicio fin
        document.querySelector(".slotHorasCobrarServicioAdd").textContent =
            obtenerHoraEuropaCentral(event.start);
        document.querySelector(".slotHoraFinCorbrarServicioAdd").textContent =
            obtenerHoraEuropaCentral(event.end);
        ...
        //empleado
        document.querySelector(".slotEmpleadoAdd").textContent =
            event.extendedProps.empleada.nombre;
        document
            .querySelector(".slotEmpleadoAdd")
            .setAttribute("data-empleid", event.extendedProps.empleada.id);
        document
    ...
    if (event.extendedProps.seleccionado_cliente) {
        console.log(
            "hay extentrop selecciona_cliente",
            event.extendedProps.seleccionado_cliente,
        );
...
function eliminarTemporalDos(idEventModify) {
    ...
    if (eventRemoveTemporalDos && eventRemoveTemporalDos.classNames) {
        ...
        eventRemoveTemporalDos.setProp(
            "classNames",
            eventRemoveTemporalDos.classNames,
        ); // Esto actualiza las clases en el DOM del evento
    }
}
...
function marcarHoraSeleccionada(contenedorHoras, horaSeleccionada) {
    ...
    $(contenedorHoras + " .index_--selected_oUDGp").removeClass(
        "index_--selected_oUDGp",
    );
    $(contenedorHoras + " .index_--highlighted__3J43").removeClass(
        "index_--highlighted__3J43",
    );

    // Buscar el elemento con la hora seleccionada
    let horaInicioSeleccionada = ContenedorHorasInicio.querySelector(
        `[data-time="${horaSeleccionada.trim()}"]`,
    );

    if (horaInicioSeleccionada) {
        $(horaInicioSeleccionada).addClass(
...
function guardarEditNewReserv() {
    const buttonModifyNewReserv = document.getElementById(
        "uid-377-inputEditNewReserv",
    );
    if (buttonModifyNewReserv) {
        buttonModifyNewReserv.addEventListener("click", function () {
            ...
            let indexArrayModificarTex = document.querySelector(
                ".allServicesAddCalendar00Add .totalServiciosAñadidos",
            ).textContent;
            let indexArrayModificar = indexArrayModificarTex.split("#")[1];
            indexArrayModificar = parseInt(indexArrayModificar) - 1;
            modifyServiceByIndex(
                indexArrayModificar,
                parseInt(idServivioAddTarjetaBlanca.trim(), 10),
            );
            ...
            getServicesById(selectedServiceIds2, function (servicios) {
                let borderColor = document.querySelector(
                    ".selectServiceAddCalendar .services_serviceDecorator_ldMxA",
                ).style.borderColor;
                borderColor = borderColor.trim();
                let nombreServicio = document.querySelector(
                    ".selectServiceAddCalendar .services_serviceName_YhbTW_span",
                ).textContent;
                nombreServicio = nombreServicio.trim();
                let horaInicio = document
                    .querySelector(".slotHorasCobrarServicioAddCalendar")
                    .textContent.trim();
                let horaFin = document
                ...
                let idEmpleado = document
                    ...
                    .getAttribute("data-empleid")
                    .trim();
                let nombre_Empleado = document
                    .querySelector(".slotEmpleadoAddInicioCalendarAdd")
                    .textContent.trim();
                let precioServicio1 = document
                    .querySelector(
                        ".selectServiceAddCalendar .services_servicePrice_wErzf",
                    )
                    .textContent.trim();
                precioServicio1 = precioServicio1.replace("€", "");
                document.querySelector(".newReservCalendar00").style.display =
                    "block";
                resetAddServiceScreen(
                    ".selectServiceAddCalendar",
                    ".slotEmpleadoAddInicioCalendarAdd",
                );
                document.querySelector(
                    ".allServicesAddCalendar00Add",
                ).style.display = "none";

                let botonesCreateReserv = document.querySelector(
                    ".buttonSavetrackModifyNewReserv",
                );
                botonesCreateReserv.style.display = "flex";
                let botonesModifyService = document.querySelector(
                    ".buttonEditStrackModifyNewReserv",
                );
                ...
                let divTarjetasIniciales = document.querySelector(
                    ".tarjetasIncialesMostrarOcultarCalendar",
                );
                divTarjetasIniciales.classList.add("d-none");
                let divTarjetasNuevas = document.querySelector(
                    ".nuevasTarjetasMostrarOcultarCalendar",
                );
                let htmlContent = "";
                let totalPricePay = 0;
                ...
                getAllEmpleados(function (empleadosReservas) {
                    ...
                    let eventoExtentropModificar =
                        calendar.getEventById(idEventModify); //idEventModify viene de showModifyService
                    let fechaParaCrearIdUnico = document
                        .getElementById("datePikerfechaCitaInfo22")
                        .getAttribute("data-datepiker");
                    let fechaIdUnico = formatearFeAnio(fechaParaCrearIdUnico);
                    let id_servicioParaIdUnico = document
                        .querySelector(".selectServiceAddCalendar")
                        .getAttribute("data-service");
                    ...
                    if (
                        eventoExtentropModificar.extendedProps &&
                        eventoExtentropModificar.extendedProps.servicio
                    ) {
                        // console.log(eventoExtentropModificar.extendedProps.id_unico, "antes de modificar id unico");

                        eventoExtentropModificar.extendedProps.servicio.borderColor =
                            borderColor;
                        eventoExtentropModificar.extendedProps.servicio.nombre =
                            nombreServicio;
                        eventoExtentropModificar.extendedProps.servicio.duracion =
                            calcularDuracion(horaInicio, horaFin);
                        eventoExtentropModificar.extendedProps.servicio.precio =
                            precioServicio1;
                        eventoExtentropModificar.extendedProps.servicio.id =
                            idServivioAddTarjetaBlanca;
                        eventoExtentropModificar.extendedProps.empleada.id =
                            idEmpleado;
                        eventoExtentropModificar.extendedProps.empleada.nombre =
                            nombre_Empleado;
                        eventoExtentropModificar.extendedProps.horaInicio =
                            horaInicio;
                        eventoExtentropModificar.extendedProps.horaFin =
                            horaFin;
                        //eliminar el evento de infoArrayEnvio y añadir el modificado
                        let index = infoArrayEnvio.findIndex(
                            (item) => item.id === idEventModify,
                        );

                        if (index !== -1) {
                            // Reemplazamos el elemento en ese índice con el nuevo elemento
                            infoArrayEnvio.splice(
                                index,
                                1,
                                eventoExtentropModificar,
                            );
                        }
                    } else {
                    ...
                    servicesWithTimes.forEach((service) => {
                        // Buscamos el empleado correspondiente usando el id_empleado
                        let empleado = empleadosReservas.find(
                            (emp) => emp.id === parseInt(service.id_empleado),
                        );
                        if (empleado) {
                            // Obtenemos las primeras dos letras del nombre del empleado
                            let iniciales = empleado.nombre
                    ...
                    let idsReservasServices = servicesWithTimes.map(
                        (s) => s.id_reserva,
                    );
                    ...
                    servicios.forEach((servicio, index) => {
                        ...
                        let seleccionaCliente =
                            servicesWithTimes[index].seleccionaCliente;
                    ...
                    addHtmlDivPrecioFinal(
                        totalPricePay,
                        '#newReservCalendar p[data-testid="appointment-price2"]',
                        '#newReservCalendar div[data-testid="appointment-to-be-paid2"]',
                    );

                    //añadir los idesEventos a los botones
                    let tarjetasServiciosMultiples = document.querySelectorAll(
                        ".subbookings-list_card_j4UGY",
                    );
                    colorBordeReservArray = [];
                    tarjetasServiciosMultiples.forEach((tarjeta, index) => {
                        let colorBorde = tarjeta.querySelector(
                            ".service_border_sBKgz",
                        ).style.borderColor;
...
if (buttonModifyReserv) {
    buttonModifyReserv.addEventListener("click", function () {
        ...
        getServicesById(selectedServiceIds2, function (servicios) {
            ...
            console.log(eventoExtentropModificar, "evento a modificar");
            let id_reservaEventoModificar =
                eventoExtentropModificar.extendedProps.reservaId;
            ...
            if (
                ...
                let borderColor = document.querySelector(
                    ".selectServiceAdd .services_serviceDecorator_ldMxA",
                ).style.borderColor;
                borderColor = borderColor.trim();
                let nombreServicio = document.querySelector(
                    ".selectServiceAdd .services_serviceName_YhbTW_span",
                ).textContent;
                nombreServicio = nombreServicio.trim();
                let horaInicio = document
                    .querySelector(".slotHorasCobrarServicioAdd")
                ...
                let precioServicio1 = document
                    .querySelector(
                        ".selectServiceAdd .services_servicePrice_wErzf",
                    )
                    .textContent.trim();
                precioServicio1 = precioServicio1.replace("€", "");
                eventoExtentropModificar.extendedProps.servicio.borderColor =
                ...
                eventoExtentropModificar.extendedProps.seleccionado_cliente =
                    seleccionadoPorCliente;
            } else {
            ...
            let botonesCobrar = document.querySelector(
                ".reservCobrarFooterInfo",
            );
            let botonesGuardarCobrar = document.querySelector(
                ".saveChangesFooterInfo",
            );
            botonesCobrar.style.display = "none";
            botonesGuardarCobrar.style.display = "flex";
            let botonesCreateReserv =
                document.querySelector(".buttonAddStrack");
            botonesCreateReserv.style.display = "flex";
            let botonesModifyService =
                document.querySelector(".buttonEditStrack");
            botonesModifyService.style.display = "none";
            ...
            getAllEmpleados(function (empleadosReservas) {
                ...
                servicios.forEach((servicio, index) => {
                    console.log(
                        idsEventosParaHtml[index],
                        "IDS EVENTOS HTML 2955",
                    );
                    // Convertir el precio a número flotante
                    let precioNumerico = parseFloat(servicio.precio);
...
if (buttonAddServiceAdd) {
    buttonAddServiceAdd.addEventListener("click", function (event) {
        ...
        getServicesById(selectedServiceIds2, function (servicios) {
            console.log(
                serviciosEliminadosTemporales.length,
                "servicios eliminados",
            );

            let duracion = convertirAHorasMinutos(
                document.querySelector(
                    ".selectServiceAdd .services_serviceDuration_Zb36z",
                ).textContent,
            );
            let noIdReserva = true;
            meterHorasArraySecundaria(
                ".slotHorasCobrarServicioAdd",
                ".slotHoraFinCorbrarServicioAdd",
                ".slotEmpleadoAdd",
                duracion,
...
function toggleOffcanvas(closeId, openId) {
    ...
    const closeOffcanvas = bootstrap.Offcanvas.getInstance(
    ...
    const openOffcanvas = new bootstrap.Offcanvas(
        document.getElementById(openId),
    );
    openOffcanvas.show();
}

// CLICK TARJETA INICIAL MANEJA APERTURA SHOWALLSERVICES MODIFICAR SERVICIO INICIAL
const serviceInfoClosedDetailsModal = document.querySelector(
    ".services_serviceInfo_iDMQw",
);
...
function cancelEditService() {
    ...
    if (newReserv === "false" || newReserv === false) {
        ...
        let idEventoAntiguo =
            botonEditarServicioReserva.getAttribute("data-index");
        // console.log(idEventoAntiguo, "idEventoAntiguo");
        let eventoEncontrado = infoArrayEnvio.find(
            (evento) => evento.id === idEventoAntiguo,
        );

        //volvemos a poner la apariencia visual del antiguo servicio
        let data = eventoEncontrado.extendedProps;
        let eventoTemporalCalendario2 = document.querySelector(
            ".fc-event.temporal2",
        );
        if (eventoTemporalCalendario2) {
    ...
    let visualizaNEmpleado = document.querySelector(
        ".visualizadorNombreEmpleado",
    );
    ...
    if (eventIdChangeCalendar === "") {
        ...
        infoArrayEnvio = infoArrayEnvio.filter(
            (item) => item.id !== eventIdChangeCalendar,
        );
        eliminarEventoCalendario(eventIdChangeCalendar);
        eventIdChangeCalendar = "";
    }
    abrirOffcanvas("eventDetailsModal");
    let botonesGuardarModificacion =
        document.querySelector(".buttonEditStrack");
...
const exitShowAllServices = document.querySelectorAll(
    ".exitShowAllServicesChange",
);
...
const cancelShowAllServices2 = document.getElementById(
    "uid-376-inputEditReserv",
);
...
if (ShowAddServices) {
    ShowAddServices.addEventListener("click", function () {
        ...
        document.querySelector(".slotEmpleadoAddInicio").textContent =
            "Selecciona empleado";
        document
            .querySelector(".slotEmpleadoAddInicio")
            .setAttribute("data-empleid", "cualquiera");
        ...
        let divTotalServicios = document.querySelector(
            ".totalServiciosAñadidos",
        );
        divTotalServicios.textContent = `Servicio #${totalServicios}`;
        ...
        if (servicesWithTimes.length >= 2) {
            // SERVICIO MULTIPLE O LO HA SIDO
            //obtener última posicion array
            horaInicio =
                servicesWithTimes[servicesWithTimes.length - 1].horaFin;
            ...
            let duracion = calcularDuracion(
                document.querySelector(".slotHorasCobrarServicio").textContent,
                document.querySelector(".slotHoraFinCorbrarServicio")
                    .textContent,
            );
            meterHorasArrayInicio(
                ".slotHorasCobrarServicio",
            ...
            setTimeout(() => {
                ...
                servicesWithTimes[0].id_empleado = document
                    .querySelector(".slotEmpleadoAddInicio")
                    .getAttribute("data-empleado");
            }, 400);
            horaInicio = obtenerHoraFinInicio();
        ...
        document.querySelector(".slotHoraFinCorbrarServicioAdd").textContent =
            calcularHoraFin(horaInicio, "30min");
        document.querySelector(".slotHorasCobrarServicioAdd").textContent =
            horaInicio;
        marcarHoraSeleccionada(
            ".contenedorHorasFinAdd",
...
function openCloseComboStatus() {
    let botonAbrirComboStatus = document.querySelector(
        ".botonAbrirComboStatus",
    );
...
function setClass(status, div, confirmacionCambioReserva) {
    ...
    let comunClass =
        "headerInfoService offcanvas-header align-items-center d-flex position-relative header_header_T53u1";
    ...
    if (confirmacionCambioReserva === "pendiente") {
        ...
        document.querySelector(".reservStatus").textContent =
            "El cliente tiene que confirmar";
    } else {
        if (status === "pending") {
            ...
            headerInfoService.className =
                "headerInfoService purify_Ks8Q8dHEaaaFeDYdNtADtw==";
        }
    }
}
...
function actionPresButon(butonAction) {
    ...
    if (butonAction === "cancelarCitaOption") {
        ...
        let cancelledButon = document.querySelector(
            ".cancelledReservButton2030",
        );
        if (cancelledButon) {
                ...
                $.ajax({
                    ...
                    data: {
                        ...
                        responsablecancelacion:
                            valoresCancelacion.responsableNombre,
                    ...
                    success: function (data) {
                        if (data.cancelada) {
                            ...
                            let stylos =
                                "position: absolute;right: 5rem;top: 16px;z-index: 9;";
                            insertMessageResolAction(
            ...
            success: function (data) {
                if (data.creada) {
                    ...
                    let stylos =
                        "position: absolute;right: 5rem;top: 16px;z-index: 9;";
                    insertMessageResolAction(
                        "Inasistencia del cliente guarda",
                        ".cal-wrapper",
                        stylos,
                        "ok",
                    );
...
function obtenerValoresCancelacion() {
    let motivoCancelacion = document.getElementById(
        "motivoCancelacion001",
    ).value;
    let inputSelectResponsable = document.querySelector(
        "#reseponsableCancelacion001Selected .vscomp-hidden-input",
    );
    let DivnombreResponsable = document.querySelector(
        "#reseponsableCancelacion001Selected .vscomp-value",
    );
...
function cancelledReservB(boton) {
    ...
    const divReserva = document.querySelector(
        `.cardAll[data-idReserva="${idReservaCancelled}"]`,
    );
    const redPoindReservDelete = document.querySelector(
        `.redPoinNewReserv[data-reservredpoindid="${idReservaCancelled}"]`,
    );
        ...
        success: function (data) {
            if (data.cancelada) {
                ...
                const notificationLink = document.querySelector(
                    ".notificationNewReserv",
                );

                // Accede al último elemento <b> dentro de "notificationNewReserv"
                const lastBElement =
                    notificationLink.querySelector("b:last-of-type");

                // Luego, accede al <span> dentro del último <b> si existe
                const spanInsideLastB = lastBElement
                    ? lastBElement.querySelector("span")
                    : null;
...
function comfirmReservB(boton) {
    ...
    const divReserva = document.querySelector(
        `.cardAll[data-idReserva="${idReservaPendiente}"]`,
    );
    const redPoindReservDelete = document.querySelector(
        `.redPoinNewReserv[data-reservredpoindid="${idReservaPendiente}"]`,
    );
    //eliminar evento del offcanvas
    if (divReserva) {
        divReserva.remove();
        ...
        .then((data) => {
            ...
            if (data.cancelled) {
                ...
                `;
                    ...
                    divTarjetas.insertAdjacentHTML(
                        "beforeend",
                        tarjetaCancelada,
                    );
                });
            ...
            if (!data.pending && !data.cancelled) {
                divTarjetas.innerHTML =
                    '<p class="text-center">No hay avisos de reservas.</p>';
            }
        })
 ...
 */
function peticionAjaxDisponibleEmpleados(
    inicioServicio,
    duracion,
    diaSeleccionado,
    id_despleableSelecEmpleado_almohadilla,
) {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            actualizarDisponibilidadEmpleadosCalendar(
    ...
    getAllClients()
        .then(function (data) {
            ...
            clientes.forEach(function (cliente, index) {
                ...
                $(container).append(`
                <div>
                    <div data-clie="${cliente.id}" data-index="${index}" class="irenemiwebItem_client002 item_client0202 pointer ${index === 0 ? "customer-el-list_active_ffoQG" : ""}" onclick="funcionClicTrajeta(this)">
...
function mostrarEmpleadoDisponible(
    botonSelector,
    slotEmpleadoSelector,
    horaServicioSelector,
    horaFinServicioSelector,
    modalSelector,
    contenedorAtributoFecha,
) {
    ...
    if (botonAbrirModal) {
        botonAbrirModal.addEventListener("click", function (event) {
            ...
            let inicioServicio =
                document.querySelector(horaServicioSelector).textContent;
            let diaSeleccionado1 = fechaElemento.getAttribute("data-datepiker");
            let horaFinServicio = document.querySelector(
                horaFinServicioSelector,
            ).textContent;
            ...
            // Llamada AJAX
            peticionAjaxDisponibleEmpleados(
                inicioServicio,
                duracion,
                diaSeleccionado,
                modalSelector,
...
mostrarEmpleadoDisponible(
    ".styles_outerWrapper_AddEmpleInicio",
    ".slotEmpleadoAddInicio",
    ".slotHorasCobrarServicio",
    ".slotHoraFinCorbrarServicio",
    "#selectEmpleModalAddInicio",
    ".fechaCitaInfo",
);
//seleccionar empleado info reserva add
mostrarEmpleadoDisponible(
    ".styles_outerWrapper_AddEmple",
    ".slotEmpleadoAdd",
    ".slotHorasCobrarServicioAdd",
    ".slotHoraFinCorbrarServicioAdd",
    "#selectEmpleModalAdd",
...
mostrarEmpleadoDisponible(
    ".styles_outerWrapper_AddEmpleInicioCalendar",
    ".slotEmpleadoAddInicioCalendar",
    ".slotHorasCobrarServicioCalendar",
    ".slotHoraFinCorbrarServicioCalendar",
    "#selectEmpleModalAddInicioCalendar",
    ".fechaCitaInfo22",
);
//seleccionar empleado calendar new reserv add
mostrarEmpleadoDisponible(
    ".styles_outerWrapper_AddEmpleInicioCalendarAdd",
    ".slotEmpleadoAddInicioCalendarAdd",
    ".slotHorasCobrarServicioAddCalendar",
    ".slotHoraFinCorbrarServicioAddCalendar",
    "#selectEmpleModalAddInicioCalendarAdd",
    ".fechaCitaInfo22",
);
 ...
 */
function actualizarDisponibilidadEmpleadosCalendar(
    disponibilidadEmpleados,
    modal,
) {
    ...
    disponibilidadEmpleados.forEach(function (empleado) {
        ...
        const divAvatarEmpleado = document.querySelector(
            `${selectorBase} .avatarEmpleadoAdd`,
        );
        const cursorEmpleado = document.querySelector(
            `${selectorBase} .empleadoCambiarCursor`,
        );
    ...
    let cursorEmpleadoCualquiera = document.querySelector(
        `${modal} .empleadoCambiarCursorCualquiera`,
    );
    if (noDisponiblesCount === totalEmpleados) {
        if (cursorEmpleadoCualquiera) {
            ...
            let subtextDiv =
                cursorEmpleadoCualquiera.querySelector(".subtext span");
            subtextDiv.innerHTML = `
        ...
        if (cursorEmpleadoCualquiera) {
            cursorEmpleadoCualquiera.classList.remove(
                "empleadoDisabled",
                "opacity-50",
            );
...
function marcarEmpleSeleccionado(slotNombre) {
    ...
    if (nombreEmpleadoSlot1) {
        ...
        let empleadosPonerCheck =
            document.querySelectorAll(".avatarEmpleadoAdd");
        empleadosPonerCheck.forEach(function (empleado) {
            empleado.classList.remove("avatar-selected");
        });
        empleadosPonerCheck.forEach(function (empleado) {
            if (
                empleado.getAttribute("data-empleadaNombre") ===
                nombreEmpleadoSlot
            ) {
...
function selectEmpleadoAdd(
    elemento,
    empleado_id,
    empleado_nombre,
    slotNombre,
    modalAbierto,
    userId_empleado = null,
) {
    if (elemento.classList.contains("empleadoDisabled")) {
    } else {
        empleadoIdIdUnico = empleado_id;
        if (
            slotNombre.trim() === ".slotEmpleadoHistorialModificacionReservas"
        ) {
            // --- 🔥 FILTRO HISTORIAL ---
        ...
        elemento
            .querySelector(".avatarEmpleadoAdd")
            .classList.add("avatar-selected");

        let visualizadorNombreEmpleadoSeleccionado =
            document.querySelector(slotNombre);
        //si empleado es "cualquiera"
        if (empleado_id === 0) {
            visualizadorNombreEmpleadoSeleccionado.innerHTML =
                "Cualquier empleado";
            visualizadorNombreEmpleadoSeleccionado.setAttribute(
                "data-empleId",
                "cualquiera",
            );
            visualizadorNombreEmpleadoSeleccionado.setAttribute(
            ...
            let servicioSeleccionado = comprobarServicioSeleccionado(
                ".selectServiceAdd",
                "Seleccionar servicio",
            );
            ...
            if (slotNombre === ".slotEmpleadoAddInicioCalendar") {
                let empleadoEliminar = document
                    .querySelector(".slotEmpleadoAddInicioCalendar")
                    .getAttribute("data-empleid");
                if (empleadoEliminar.trim() === "cualquiera") {
                    ponerEventoInicialmente(empleado_id);
                } else {
                    cambiarResourceIdEvento(
                        eventIdChangeCalendar,
                        empleado_id,
                        ".fc-event.temporal",
                    );
                ...
                let empleadoEliminar2 = document
                    .querySelector(".slotEmpleadoAddInicioCalendarAdd")
                    .getAttribute("data-empleid");
        ...
        let existeServicio = document.querySelector(
            ".selectServiceAdd .services-wrapper_serviceEmpty_pbusk",
        );
        if (!existeServicio) {
            document.getElementById("uid-377-input").disabled = false;
            document
        ...
        let existeServicioCalendar = document.querySelector(
            ".addServiceCalendar66 .services_serviceInfo_iDMQwAddCalendar",
        );
        if (existeServicioCalendar) {
            document
                .getElementById("uid-798-input")
                .classList.remove("index_is--disabled_w97Nq");
            document
                .getElementById("uid-3978-input")
                .classList.remove("index_is--disabled_w97Nq");
            document
                .getElementById("uid-3978-input")
        ...
        if (
            document.querySelector(".allServicesAddCalendar00Add").style
                .display === "block"
        ) {
            let servicioSeleccionado = comprobarServicioSeleccionado(
                ".selectServiceAddCalendar",
...
function createTicket(
    ...
    return new Promise(function (resolve, reject) {
        ...
        if (infoArrayEnvio.length) {
            id_reservaRecibo = infoArrayEnvio.map(
                (evento) => evento.extendedProps.reservaId,
            )[0];
            ...
            !document
                .querySelector(".basket-layout_sidebar_X6qEm")
                .getAttribute("data-idreserv")
        ) {
            id_reservaRecibo = 0; // venta rápida
            console.log(
                id_reservaRecibo,
                "id servicio en venta rápida",
                document
                    .querySelector(".basket-layout_sidebar_X6qEm")
                    .getAttribute("data-idreserv"),
            );
        } else {
            id_reservaRecibo = document
                .querySelector(".basket-layout_sidebar_X6qEm")
                .getAttribute("data-idreserv");
        ...
        let pagoEfectivoCambio = document.querySelector(
        ...
        let metodoActivo = document
            .querySelector(".payment-types_paymentMethodActive_vBa20")
            .textContent.trim();
...
function insertTargetPayment() {
    ...
    if (reservasGloval == null && infoArrayEnvio.length) {
        reservasGloval =
            transformarEventosMultiplesFormatoFinal(infoArrayEnvio);
    }
        ...
        .then((data) => {
            ...
            const metodoActivo =
                document
                    .querySelector(".payment-types_paymentMethodActive_vBa20")
                    ?.textContent.trim() || "";
            const divCliente = document.querySelector(
                ".customer-card_customer_PiI9d",
            );
            const ahora = new Date();

            const fecha_hora = ahora
            ...
            if (pagoEfectivoCambio?.classList.contains("d-none")) {
                ...
                cantidadCambioMostrarOcultar = convertirEnNumeroSolo(
                    document.querySelector(".basketTotalPrecio")?.textContent ||
                        "0",
                )
                    .toFixed(2)
                    .replace(".", ",");
                ...
                // PAGO NO COMBINADO
                const valorInput =
                    document.getElementById("uid-317-inputMetodoPago")?.value ||
                    "0";
                const cambioTexto =
                    document.querySelector(".cambio_800")?.textContent || "0";

                cantidadCambioMostrarOcultar = convertirEnNumeroSolo(valorInput)
                    .toFixed(2)
                    .replace(".", ",");
                cambioMostrarOcultar = convertirEnNumeroSolo(cambioTexto)
                    .toFixed(2)
                    .replace(".", ",");
            }
            ...
            if (divCliente) {
                ...
                getClientById(id_cliente)
                    .then((cliente) => {
                        ...
                        const datosCliente =
                            montarTarjetaClienteTiket(cliente99);
    ...
    const reservas = eventos.map((ev) => {
        ...
        return {
            ...
            user: {
                ...
                device_token: null,
                profile_photo_url: `https://ui-avatars.com/api/?name=${p.usuario.nombre[0]}&color=7F9CF5&background=EBF4FF`,
            },
        ...
        reservas: [
            {
                ...
                user: {
                    ...
                    cliente_confianza: "No",
                    device_token: null,
                    profile_photo_url: `https://ui-avatars.com/api/?name=${props.usuario.nombre[0]}&color=7F9CF5&background=EBF4FF`,
                },
...
function activarLoaderAll() {
    document
        .getElementById("loaderSperaAdministratorAll")
...
function montarTarjetaTiket(
    ...
    let divContenedorTicket = document.querySelector(
        ".index_checkoutView_oS9m6Secundario",
    );
    let divContenedorTicketPrincipal = document.querySelector(
        ".index_checkoutView_oS9m6Principal",
    );
    let serviciosTicket = "";
    getServicesById(serviciosVentaRapida_ids, function (servicios) {
        ...
        let valor_bruto =
            document.querySelector(".basketTotalPrecio").textContent; //total del tiket
        let resultadoIvaTicket = calcularIva(
            convertirEnNumeroSolo(valor_bruto),
            0.21,
        );
        let descuentoTotalPorcentaje = document.querySelector(
            ".basket-discountPorcentajeShow",
        ).textContent;
        let descuentoTotalImporte = document.querySelector(
            ".basket-discountShow",
        ).textContent;
        ...
        createTicket(
            cliente99.id,
            resultadoIvaTicket.importeIva,
            resultadoIvaTicket.valorNeto,
            valor_bruto,
            subtotalTiket,
            descuentoTotalImporte,
            descuentoTotalPorcentaje,
        )
        ...
        `);
                ...
                $("#uid-340-inputEnviarEmail").on("click", function () {
                    activarLoaderAll();
                    let emailCliente = document.getElementById(
                        "emailClienteRecivoSend",
                    ).value;
                    ...
                    $.ajax({
                        ...
                        data: {
                            ...
                            tipoIva: tipoIva,
                            valor_neto: resultadoIvaTicket.valorNeto
                                .toString()
                                .replace(".", ","),
                            importe_iva: resultadoIvaTicket.importeIva
                                .toString()
                                .replace(".", ","),
                        ...
                        success: function (response) {
                            if (response.enviado === true) {
                                ...
                                document
                                    .querySelector(".enviarReciboCorreo")
                                    .classList.add("d-none");
                                document
                                    .querySelector(".botonesCabeceraTiquet ")
                                    .classList.remove("d-none");
                                let stylos =
                                    "position: absolute;right: 5rem;top: 16px;z-index: 9;";
                                insertMessageResolAction(
                                    "Revibo enviado con éxito",
                                    ".index_checkoutView_oS9m6Secundario",
                                    stylos,
                                    "ok",
                                );
                                ...
                                insertMessageResolAction(
                                    "No hemos podido enviar el recibo",
                                    ".index_checkoutView_oS9m6Secundario",
                                    stylos,
                                    "error",
                                );
...
function actionButoncloseOffcanvasSelectClient(id_input = null) {
    ...
    let offcanvasSelectClientes = document.getElementById(
        "offcanvasSelectClient",
    );
    if (
        offcanvasSelectClientes.classList.contains(
            "card_empty_calendarOffcanvas",
        )
    ) {
        $("#newReservCalendar").offcanvas("show");
    }
    if (
        offcanvasSelectClientes.classList.contains("card_empty_InfoOffcanvas")
    ) {
...
function clicTarjetasBlancasSelectCliente(tarjetaVacia) {
    ...
    if (tarjetaVacia) {
        ...
        offcanvas.classList.remove(
            "card_empty_ventasOffcanvas",
            "card_empty_calendarOffcanvas",
            "card_empty_InfoOffcanvas",
        );
...
function insertarTarjetaClienteSelecionado(
    cliente,
    divInfoClienteEnviado,
    claseDiferenciadora,
) {
...
function insertarTarjetaClienteSelecionadoExtentrop(
    cliente,
    divInfoClienteEnviado,
    claseDiferenciadora,
) {
    ...
    getClientById(id_cliente)
        .then(function (cliente) {
            ...
            if (accionResolver.trim() === "historialModificacionReserva") {
                ...
                accionResolver.trim() ===
                "historialModificacionReservaResponsable"
            ) {
                ...
                if (
                    document
                        .getElementById("offcanvasSelectClient")
                        .classList.contains("card_empty_calendarOffcanvas")
                ) {
                    // //console.log("cliente calendario");
                    $("#newReservCalendar").offcanvas("show");
                    insertarTarjetaClienteSelecionado(
                        cliente,
                        ".basket-customer-card0101Calendar",
                        "card_empty_calendar",
                    );
                    ...
                    document
                        .getElementById("offcanvasSelectClient")
                        .classList.contains("card_empty_ventasOffcanvas")
                ) {
                    insertarTarjetaClienteSelecionado(
                        cliente,
                        ".basket-customer-card0101",
                        "card_empty_ventas",
                    );
                    ...
                    document
                        .getElementById("offcanvasSelectClient")
                        .classList.contains("card_empty_InfoOffcanvas")
                ) {
                    insertarTarjetaClienteSelecionado(
                        cliente,
                        ".basket-customer-card0101Info",
...
function workAsig002(servicio_id, indexModalVentaRapida, precioServicio) {
    ...
    console.log(
        document
            .querySelector(".service-variant_item_Cye7B")
            .getAttribute("data-bs-toggle"),
        "valor toggle",
    );
    if (
        !document
...
function selectEmpleAMark(
    indexDivVisualizador = null,
    empleNombreAsig,
    elemento,
    idEmpleado,
    claseDesplegable = null,
    divNombreEmpleado = null,
    titleEmpleado = null,
) {
    ...
    if (indexDivVisualizador === null) {
        ...
        desplegableEmpleados = document.querySelector(
            `.droponAbrirEmpleA${indexDivVisualizador}`,
        );
        if (desplegableEmpleados) {
            desplegableEmpleados.classList.add("d-none");
        }
        divNOmbreEmpleA = document.querySelector(
            `.nombreEmpleadoA${indexDivVisualizador}`,
        );
        titleName = document.querySelector(
            `.titleEmpleadoA${indexDivVisualizador}`,
        );
    }
    ...
    const empleadosConTick = document.querySelectorAll(
        ".select-staffer_highlight_tt5tB .b-icon.iconFont.icon-tick",
    );
...
function selectEmpleBotonCambiar(nombreEmpleado, idEmpleado) {
    ...
    if (encargadoCobro === "") {
        ...
        let liElemento = document.querySelector(
            `.basket-transactions-list li[data-index="${index_li}"]`,
        );
        ...
        let divNombre_avatar = liElemento.querySelector(
            ".assigned-staffer_assignedStaffer_cmkGT",
        );
...
function grupoBotonesMostrarVentaRapidan(selectorBotonMostrar) {
    ...
    const botonesVentaRapidaEna = document.querySelector(
        ".botonesEnabledVentaRapida",
    );
    const botonSoloPapeleraDisable = document.querySelector(
        ".botonesEnabledVentaRapidaSoloPapelera",
    );
    const botonesVentaRapiDisabled = document.querySelector(
        ".botonesDisabledVentaRapida",
    );

    if (botonesVentaRapidaEna) botonesVentaRapidaEna.classList.add("d-none");
    if (botonSoloPapeleraDisable)
        botonSoloPapeleraDisable.classList.add("d-none");
    if (botonesVentaRapiDisabled)
...
function openModalEmpleA(desplegableindex) {
    let desplegableAbrirEmpleA = document.querySelector(
        `.droponAbrirEmpleA${desplegableindex}`,
    );
...
function openModalEmpleAreservNoPay() {
    let desplegableAbrirEmpleA = document.querySelector(
        `.droponAbrirEmpleadoReservNoPay`,
    );
    desplegableAbrirEmpleA.classList.toggle("d-none"); // La clase 'd-none' oculta el div
}
...
function openModalEmpleDesdeBoton(listaNombreCambiar) {
    let desplegableAbrirEmpleA = document.querySelector(
        `.droponAbrirEmpleBotonCambiar`,
    );
    desplegableAbrirEmpleA.classList.toggle("d-none"); // La clase 'd-none' oculta el div
}
function construirTarjetaServiciosTiket(
    indexli,
    nombreServicio,
    duracionServicio,
    precio,
...
function construirHtmlTarjetasVentaRapida(
    indexli,
    nombreServicio,
    duracionServicio,
    precio,
    nombreApellidosEmpleado,
    idSErvicio,
    discountValue,
) {
   ...
   <li data-index="${indexli}" data-servicio="${idSErvicio}" data-discount="${discountValue}">
        ...
        discountHtml +
        `
            <div data-testid="assigned-staffer" class="assigned-staffer_assignedStaffer_cmkGT">` +
        seccionAbatar +
        `
...
function showEditBasket(liElement_index) {
    let liElemento = document.querySelector(
        `.basket-transactions-list li[data-index="${liElement_index}"]`,
    );
    ...
    let porcentaje = liElemento.querySelector(
        ".porcentajeDescuento",
    ).textContent;
    ...
    if (descuentosVentaRapida[parseInt(liElement_index)]) {
        // Si el índice existe, modificar el valor
        importeDescuento11 =
            descuentosVentaRapida[parseInt(liElement_index)].descuentoServicio;
    }
    inputTotalDescuentoServicio.value = importeDescuento11;
    let nameService = liElemento.querySelector(
        ".basket-row_basketRowName_HCisM",
    ).textContent;
...
function actualizarTotalSubtotal() {
    ...
    let totalDescuentosServicios = descuentosVentaRapida.reduce(
        (total, item) => {
    ...
    let input_uid317inputMetodoPago = document.getElementById(
        "uid-317-inputMetodoPago",
    );
    dibSubtotal.textContent =
        totalBasquetSubtotal.toFixed(2).replace(".", ",") + "€";
    totalPrecio.textContent =
        totalBasquetTotal.toFixed(2).replace(".", ",") + "€";
    input_uid317inputMetodoPago.value = totalBasquetTotal
        .toFixed(2)
        .replace(".", ",");
}
...
function createIconCorazon() {
    ...
    iconImgCorazon.src =
        urlAplicacion + "/storage/calendar/corazonRojoPequenio.svg";
...
function insertDateService(servicio_id, indexModalVentaRapida, precioServicio) {
    ...
    let nombreVentaRapida = document.querySelector(
        `.nombreEmpleadoA${indexModalVentaRapida}`,
    ).textContent;
    let id_empleado = document
        .querySelector(`.nombreEmpleadoA${indexModalVentaRapida}`)
    ...
    let listaTransacciones = document.querySelector(
        ".basket-transactions-list",
    );
    ...
    if (cestoLleno) {
        ...
        getServicesById(serviciosVentaRapida_ids, function (servicios) {
            servicios.forEach((servicio, index) => {
                ...
                let li_index = index;
                let servicio_idArray = serviciosVentaRapida[index].idServicio;
                let nombreApellidoEmpleA =
                    serviciosVentaRapida[index].nombre_Empleado;
                let precioAsignar = serviciosVentaRapida[index].precio;
    ...
    if (responsableSeleccionado.trim() === "No hay asignación de personal") {
        ...
        grupoBotonesMostrarVentaRapidan(
            ".botonesEnabledVentaRapidaSoloPapelera",
        );
    } else {
...
function showEmpleDesple(boton) {
    let modalSelecEmpleaBoton = document.getElementById(
        "droponAbrirEmpleBotonCambiar",
    );
    ...
    if (encargadoCobro) {
        modalSelecEmpleaBoton.setAttribute(
            "data-encargadoCobro",
            encargadoCobro,
        );
...
if (botonRemoveItemVentaRapida) {
    botonRemoveItemVentaRapida.addEventListener("click", function (event) {
        event.preventDefault();
        let index_li = document
            .getElementById("modificarArticulo")
            .getAttribute("data-lista-li");

        let elemento_li = document.querySelector(
            `.basket-transactions-list li[data-index="${index_li}"]`,
        );
        // Recorrer el array
...
function updateDiscount() {
    ...
    const price =
        parseFloat(document.getElementById("uid-317-input").value) || 0; // El precio modal
    const discountRate =
        parseFloat(
            document.getElementById("uid-319-input").value.replace("%", ""),
        ) || 0; // El porcentaje de descuento
...
function actualizarPorcentajeTotal() {
    let divPorcentaje33 = document.querySelector(
        ".basket-discountPorcentajeShow",
    ).textContent;
...
function actualizarDescuentoTotal() {
    ...
    let porcentaje115 = document.querySelector(
        ".basket-discountPorcentajeShow",
    ).textContent;
    ...
    actualizarTotalSubtotal();
    document.querySelector(".basket-discountShow").textContent =
        discountTotalPrice115 + "€";
}

//boton guardar descuento total
let botonDescuentoTotal = document.getElementById(
    "uid-guardarDescuentoTotal-input",
);
if (botonDescuentoTotal) {
    botonDescuentoTotal.addEventListener("click", function (event) {
        event.preventDefault();

        let porcentajeDescuentoTotal = document.getElementById(
            "uid-totalDescuento-input",
        ).value;
        let subtotalDescuentoTotal =
            document.querySelector(".divSubtotal").textContent;
        ...
        const discountTotalPrice =
            (subtotalDescuentoTotal * porcentajeDescuentoTotal) / 100;
        let formattedPrice = new Intl.NumberFormat("de-DE", {
            minimumFractionDigits: 2,
            maximumFractionDigits: 2,
        }).format(discountTotalPrice);
        let visualizaImporteDescuentoTotal = document.querySelector(
            ".basket-discountShow",
        );
        descuentoTotal = discountTotalPrice;

        visualizaImporteDescuentoTotal.textContent = formattedPrice + "€";
        document.querySelector(".basket-discountPorcentajeShow").textContent =
...
function continueButtonPayment() {
    ...
    let div_HlQSH = document.querySelector(
        ".basket-layout_basketWrapper_HlQSH",
    );
    let input_uid317inputMetodoPago = document.getElementById(
        "uid-317-inputMetodoPago",
    );
...
var metodosPado = document.querySelectorAll(
    ".margin-bottom32MetodoPago .payment-types_col_ius7E",
);
metodosPado.forEach(function (metodo) {
    metodo.addEventListener("click", function (event) {
        ...
        let inputPreciApagar = document.getElementById(
            "uid-317-inputMetodoPago",
        );
        inputPreciApagar.setAttribute("data-type", tipoPago);
        inputPreciApagar.value = importePago;
        document
            .querySelector(".tipoPago2")
            ?.setAttribute("data-tipoPago2", tipoPago);
        ...
        document
            .querySelector(".payment-types_paymentMethodActive_vBa20")
            ?.classList.remove("payment-types_paymentMethodActive_vBa20");

        // Luego, agregar la clase al div contiguo dentro del método clicado
        metodo
            .querySelector("div")
            ?.classList.add("payment-types_paymentMethodActive_vBa20");
...
if (inputPreciApagar) {
    inputPreciApagar.addEventListener("click", function (event) {
        ...
        validarBorde();
        calcularCambio();
        let tipoPago = document
            .querySelector(".tipoPago2")
            .getAttribute("data-tipopago2");

        if (tipoPago === "efectivo") {
            var myModal = new bootstrap.Modal(
                document.getElementById("modalIntroducirImporte33"),
            );
            myModal.show();
        } else if (
            tipoPago === "suscirpcion" ||
            tipoPago === "tarjetaRegalo" ||
            tipoPago === "bonoSesiones"
        ) {
            //modal encontrar tarjeta
        } else {
            var myModal = new bootstrap.Modal(
                document.getElementById("modalIntroducirImporte33Tarjetas"),
            );
            myModal.show();
            let importePago =
                document.querySelector(".basketTotalPrecio").textContent;
...
function resetModalTarjeta() {
    document.getElementById("uid-730-input").value =
        document.querySelector(".basketTotalPrecio").textContent;
...
if (inputImporteTarjeta) {
    inputImporteTarjeta.addEventListener("input", function () {
        var importeExterior = convertirEnNumeroSolo(
            document.querySelector(".basketTotalPrecio").textContent,
        );
        ...
        let cantidadCero_inferior = document.querySelector(
            ".cantidadCero_inferior",
        );
        let botonHabilitado = document.getElementById("uid-3039-inputTarjeta");
        if (importeInterior > importeExterior) {
            cantidadCero_inferior.textContent =
                "El importe no puede superar la cantidad de: " +
                importeExterior.toFixed(2).replace(".", ",") +
                "€";
...
function mostrarModalOpciones007(id_input) {
    document
        .getElementById("modalCambiarMetodoPago")
        .setAttribute("data-idImput", id_input);
    ...
    var elementos0069 = document.querySelectorAll(
        "#modalCambiarMetodoPago .metodoPago004",
    );
    ...
    var myModal = new bootstrap.Modal(
        document.getElementById("modalCambiarMetodoPago"),
    );
    myModal.show(); // Abre el modal
}

//clic en tarjetas modal cambio método pago
var elementos0070 = document.querySelectorAll(
    "#modalCambiarMetodoPago .metodoPago004",
);
if (elementos0070) {
    // Iteramos sobre los elementos encontrados y mostramos el contenido de cada uno
    elementos0070.forEach(function (elemento0070) {
        elemento0070.addEventListener("click", function (event) {
            let id_input0006 = document
                .getElementById("modalCambiarMetodoPago")
                .getAttribute("data-idimput");
            //console.log(elemento0070.textContent.trim()," clic en tarjeta", id_input0006, "id del imput");
            document.getElementById(id_input0006).value =
                elemento0070.textContent.trim();
            // event.preventDefault();
...
function aniadirActiveMetodoPago(dataType) {
    var metodoActivo = document.querySelector(
        ".payment-types_paymentMethodActive_vBa20",
    );
    if (metodoActivo) {
        metodoActivo.classList.remove(
            "payment-types_paymentMethodActive_vBa20",
        );
    }
...
var botonGuardarImporteTarjeta = document.getElementById(
    "uid-3039-inputTarjeta",
);
if (botonGuardarImporteTarjeta) {
    botonGuardarImporteTarjeta.addEventListener("click", function (event) {
        ...
        let importeTrajeta120 = convertirEnNumeroSolo(
            inputImporteTarjeta120.value,
        );
        ...
        let tipoPago033 = document
            .querySelector(".tipoPago2")
            .getAttribute("data-tipoPago2");
        if (inputImporteTarjeta.value < 1) {
            cantidadCero_inferior.textContent =
                "Introduce una cantidad superior a 0€";
            cantidadCero_inferior.classList.remove("d-none");
            ...
            cantidadCero_inferior.classList.add("d-none");
            botonHabilitado.classList.remove("boton_deshabilitado");
            let importeRestante01 =
                document.querySelector(".divInputRestante").textContent;
...
function comprobarResto() {
    ...
    let importeTotalCestaRestante =
        document.querySelector(".basketTotalPrecio").textContent;
    let valorInputEntrada126 = convertirEnNumeroSolo(inputEntrada126.value);
    importeTotalCestaRestante = convertirEnNumeroSolo(
        importeTotalCestaRestante,
    );
    ...
    divRestante.textContent =
        restante >= 0 ? restante.toFixed(2).replace(".", ",") + "€" : "0,00€";
}
...
let botonCancelarpagotarjeta55 = document.getElementById(
    "uid-223-input-cancelarCantidadTarjetas",
);
...
let botonCancelarpagoEfectivo20 = document.getElementById(
    "uid-223-input-cancelarCantidad",
);
...
var bordeCambiarColor = document.querySelector(
    ".styles_outerWrapper_NumuGIntroducirImporte",
);
...
if (botonGuardarPrecioIntroducido) {
    botonGuardarPrecioIntroducido.addEventListener("click", function (event) {
        event.preventDefault();
        var inputMetodoPagovalor =
            document.getElementById("uid-3034-input").value;
        let visualizadorFinalImporte = document.getElementById(
            "uid-317-inputMetodoPago",
        );
        ...
        visualizadorFinalImporte.value =
            parseFloat(inputMetodoPagovalor).toFixed(2) + "€"; // Se muestra con dos decimales
        visualizadoCambio.textContent =
            parseFloat(inputCambiovalor).toFixed(2).replace(".", ",") + " €";
    });
}
...
const replacementDiv = document.querySelector(
    ".input-checkbox_replacement_dMLsR",
);
...
if (checkbox001) {
    checkbox001.addEventListener("change", function () {
        ...
        let listaItems410 = document.querySelectorAll(
            ".service-variant_item_Cye7B",
        );
        ...
        if (checkbox001.checked) {
            ...
            replacementDiv.classList.add(
                "input-checkbox_replacementChecked_uxNiJ",
            );
...
const checkbox002 = document.getElementById(
    "uid-288-dont-ask-againReservNotPay",
);
const replacementDiv002 = document.querySelector(
    ".input-checkbox_replacement_dMLsRNopay",
);
...
if (checkbox002) {
    checkbox002.addEventListener("change", function () {
        ...
        if (checkbox002.checked) {
            // Si está marcado, agregamos la clase al div y cambiamos el valor del input a "true"
            replacementDiv002.classList.add(
                "input-checkbox_replacementChecked_uxNiJ",
            );
            ...
            // Si no está marcado, quitamos la clase del div y cambiamos el valor del input a "false"
            replacementDiv002.classList.remove(
                "input-checkbox_replacementChecked_uxNiJ",
            );
...
function reseteoVistaVenta() {
    ...
    let contenedorTarjetasServiciosCalendar = document.querySelector(
        ".appointment-card_appointment_F_IwZ",
    );
    let div_HlQSH = document.querySelector(
        ".basket-layout_basketWrapper_HlQSH",
    );
    ...
    let pagosCombinados01 = document.querySelector(".pagoCombinado66");
    let cambioMostrar01 = document.querySelector(".cambioMostrarOcultar");
    let activarEfectivo = document.querySelector(
        '.margin-bottom32MetodoPago .payment-types_col_ius7E[data-type="efectivo"]',
    );
    let metodoActivo = document.querySelector(
        ".payment-types_paymentMethodActive_vBa20",
    );
    ...
    if (contenedorTarjetasServiciosCalendar) {
        let divContenedorFechaHora = document.querySelector(
            ".appointment-date_date_UsCxi",
        );
        if (divContenedorFechaHora) {
            $(divContenedorFechaHora).empty();
        }
        let statusReserva = document.querySelector(
            ".statusReservaCalendarCobrar",
        );
    ...
    let discountPorcentajeShow = document.querySelector(
        ".basket-discountPorcentajeShow",
    );
...
function scrollCarousel(amount) {
    const nav = document.querySelector(
        ".b-tabs_content_lxbV0OpcionesInfoCliente",
    );
...
function funcionClicTrajeta(cliente) {
    ...
    const items = document.querySelectorAll(".irenemiwebItem_client002");

    // 2️⃣ Recorrerlos y quitar la clase 'customer-el-list_active_ffoQG'
    items.forEach((item) => {
        item.classList.remove("customer-el-list_active_ffoQG");
    });
    // Añadir la clase activa al elemento clicado
    cliente.classList.add("customer-el-list_active_ffoQG");
    document
        .querySelector(".index_customerWrapper_r1idQ")
        .classList.add("list_customerCardMobileView_CfTzS");
    document
        .querySelector(".index_customerWrapper_r1idQ")
        .classList.add("cutomers-list-with-banner");
...
pestaniaProximaPasada.forEach(function (pestania) {
    pestania.addEventListener("click", function () {
        ...
        const pestaniaActiva = document.querySelector(
            ".b-tabs_tabBorderedActive_ff9lg_proximaPasada",
        );
        if (pestaniaActiva) {
            pestaniaActiva.classList.remove(
                "b-tabs_tabBorderedActive_ff9lg_proximaPasada",
            );
...
function getFristClient(idCliente) {
    ...
    if (idCliente === "primero") {
        ...
        document
            .querySelector(".item_client0202")
            .classList.add("customer-el-list_active_ffoQG");
    }
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            montarTarjetaInfoCliente(
                response.firstClient,
                response.firstClientInitials,
                response.infoAdicionalCliente,
            );

            updateClientListInfo(
                response.totalFinalizadas,
                response.totalProximas,
            );

            // Manejo de citas terminadas y próximas
            handleAppointments(
                response.terminadasFclient,
                response.serviciosTerminados,
                response.empleadasTerminadas,
                ".listaCitasClienteTerminadas003",
                ".b-emptyPassadas",
                ".listaUl_ClienteTerminadas003",
            );
            handleAppointments(
                response.proximasFclient,
                response.serviciosProximos,
                response.empleadasProximas,
                ".listaCitasClienteProximas003",
                ".b-emptyProximas",
                ".listaUl_ClienteProximas003",
            );
        },
        error: function (xhr) {
            //console.log('Error al actualizar el status', xhr);
...
function montarTarjetaInfoCliente(
    primerCliente,
    inicialesFrisclient,
    ...
    `);
    document.querySelector(".index_fullName_i2BLH").textContent =
        primerCliente.name;
    document
        .querySelector(".inicilalesCliente0050")
        .setAttribute("title", primerCliente.name);
    ...
    $(".index_visitsInfo_Uuncf").append(`
        ...
        <div class="index_visitsInfoItem_zUjfI">
            ...
            <div class="index_value_kD_oD index_size--14-sb_kg1ff index_valueTrusted_sPXxO">${infoAdicionalCliente.clienteConfianza}</div>
        </div>
     `);
}
...
function updateClientListInfo(totalFinalizadas, totalProximas) {
    document.querySelector(".li_pasada_cliente").textContent =
        `Pasadas (${totalFinalizadas})`;
    document.querySelector(".li_proxima_cliente").textContent =
        `Próximas (${totalProximas})`;

    document
        .querySelector(".b-emptyPassadas")
        .classList.toggle("d-none", totalFinalizadas > 0);
    document
        .querySelector(".listaUl_ClienteTerminadas003")
        .classList.toggle("d-none", totalFinalizadas === 0);

    document
        .querySelector(".b-emptyProximas")
        .classList.toggle("d-none", totalProximas > 0);
    document
        .querySelector(".listaUl_ClienteProximas003")
        .classList.toggle("d-none", totalProximas === 0);
}
...
function handleAppointments(
    ...
    citas.forEach(function (cita, index) {
        ...
        let formattedTotal =
            cita.total_payment.toString().replace(".", ",") + " €";

        let divPagado =
            cita.status_payment === "Pagado"
...
function getStatusClass(status) {
    let divStatusClases =
        "margin-bottom-4 style_status_xxjlV style_statusDefault_HPmTE style_status--xs_vvmA5 style_statusUpperCase_bkX7Z";
...
var flechaOcultarInfoCliente = document.querySelector(
    ".cerrarVistaInfoCliente",
);
if (flechaOcultarInfoCliente) {
    flechaOcultarInfoCliente.addEventListener("click", function () {
        document
            .querySelector(".index_customerWrapper_r1idQ")
            .classList.remove("list_customerCardMobileView_CfTzS");
        document
            .querySelector(".index_customerWrapper_r1idQ")
            .classList.remove("cutomers-list-with-banner");
    });
}
...
function buscar(inputSeachClient) {
    ...
    $(".item_client0202").each(function () {
        ...
        var textoCliente = $(this)
            .find(".customer-el-list_searchItemName_LLoTq")
            .text()
            .toUpperCase();
...
function openModalNewReservCalendar() {
    let divContenedorDropon = document.querySelector(
        ".droponNuevaCitaCalendar",
    );
    if (divContenedorDropon) {
        divContenedorDropon.classList.toggle("add-event_open_RxMza");
        let modalnewReservCalendar = document.querySelector(
            ".droponNuevaReservaCalendar",
        );
...
let botonesNewReservFaltaDispoCalendario = document.querySelectorAll(
    ".add-event_button_DtVNQ",
);
        ...
        $(boton)
            ...
            .on("click", function (event) {
                ...
                if (dataUrl === "add.reserve") {
                    ...
                    const newReservCalendarModal = new bootstrap.Offcanvas(
                        document.getElementById("newReservCalendar"),
                    );
...
function existeServicioNewReservCalendar() {
    ...
    if (document.querySelector(".services_serviceInfo_iDMQwAddCalendar")) {
        hayServicio = document
            .querySelector(".services_serviceInfo_iDMQwAddCalendar")
            .getAttribute("data-indexactual");
        if (hayServicio) {
            return true;
...
function existeNombreCliente() {
    let divTarjetaCliente = document.querySelector(
        ".basket-customer-card0101Calendar .customer-card_customerData_Ke3s5",
    );
    if (divTarjetaCliente) {
        let nombreCliente = divTarjetaCliente
            .querySelector(".b-avatar_avatar_pJzSu")
            .getAttribute("title");
...
function existeNombreClienteComun(principalClass) {
    ...
    let divTarjetaCliente = document.querySelector(
        `${principalClass} .customer-card_customerData_Ke3s5`,
    );
    if (divTarjetaCliente) {
        let nombreCliente = divTarjetaCliente
            .querySelector(".b-avatar_avatar_pJzSu")
...
function ponerEventoInicialmente(id_empleado) {
    ...
    if (existeServicioNewReservCalendar()) {
        title =
            ...
            document
                .querySelector(
                    ".services_serviceInfo_iDMQwAddCalendar .services_serviceName_YhbTW_span",
                )
                .textContent.trim();
    }
    let extendedProps = {
    ...
    if (calendar) {
        ...
        let horaInicioFormateada =
            horaInicioObj.getHours().toString().padStart(2, "0") +
            ":" + // Obtener la hora con 2 dígitos
            horaInicioObj.getMinutes().toString().padStart(2, "0") +
            ":" + // Obtener los minutos con 2 dígitos
    ...
    setTimeout(() => {
        ...
        if (eventoTemporal) {
            if (existeServicioNewReservCalendar()) {
                eventoTemporal.style.setProperty(
                    "border-left",
                    `4px solid ${colorBordeNewReservCalendar}`,
                    "important",
                );
                eventoTemporal.style.setProperty(
                    "border-top",
                    `1px solid ${colorBordeSuperior}`,
                    "important",
                );
            }
        }
...
function existeServicioPantalla2(tarjetaServicioClass) {
    ...
    if (document.querySelector(tarjetaServicioClass)) {
        hayServicio = document
            .querySelector(tarjetaServicioClass)
            .getAttribute("data-service");
        if (hayServicio) {
            return true;
        } else {
...
function ponerEventoPantalla2Info(id_empleado, empleado_nombre = null) {
    ...
    let noExisteTarjetaServicio = document.querySelector(
        ".selectServiceAdd .services-wrapper_serviceEmpty_pbusk",
    );
    ...
    if (existeServicioPantalla2(".selectServiceAdd")) {
        loaderWite();
        precioServicio1 = document.querySelector(
            ".selectServiceAdd .services_servicePrice_wErzf",
        ).textContent;
        precioServicio1 = precioServicio1.replace("€", "");
        precioServicio1 = precioServicio1.trim();
        borderColor = document.querySelector(
            ".selectServiceAdd .services_serviceDecorator_ldMxA",
        ).style.borderColor;
        borderColor = borderColor.trim();
        nombreServicio = document.querySelector(
            ".services_serviceInfo_iDMQwAdd .services_serviceName_YhbTW_span",
        ).textContent;
        nombreServicio = nombreServicio.trim();
        id_servicio = document
            .querySelector(".selectServiceAdd")
            .getAttribute("data-service");
        title =
            title +
...
function cambiarResourceIdEvento(
    eventId,
    newResourceId,
    eventoTemporalEnviado,
) {
    ...
    if (eventoTemporalEnviado === ".fc-event.temporal") {
        ...
        if (eventoTemporal1) {
            ...
            if (
                document.querySelector(".services_serviceInfo_iDMQwAddCalendar")
            ) {
                hayServicio = document
                    .querySelector(".services_serviceInfo_iDMQwAddCalendar")
                    .getAttribute("data-indexactual");
                if (hayServicio) {
                    eventoTemporal1.style.setProperty(
                        "border-left",
                        `4px solid ${colorBordeNewReservCalendar}`,
                        "important",
                    );
        ...
        if (eventoTemporal2) {
            ...
            if (document.querySelector(".selectServiceAddCalendar")) {
                ...
                let hayServicio2 = document
                    .querySelector(".selectServiceAddCalendar")
                    .getAttribute("data-service");
                let hayServicio2ChangeInfo = document
                    .querySelector(".selectServiceAdd")
                    .getAttribute("data-service");
                if (hayServicio2) {
                    //console.log("hay servicio en change empleado");

                    eventoTemporal2.style.setProperty(
                        "border-left",
                        `4px solid ${colorBordeNewReservCalendar}`,
                        "important",
                    );
            ...
            if (eventoTemporal1) {
                ...
                if (calendar.getEventById(eventIdChangeCalendar)) {
                    ...
                    let eventoElement1 = id_eventoEditar2.replace(
                        /^eventoTemporal_/,
                        "eventoTemporalAsignado_",
                    );
                    let eventoElement = document.querySelector(
                        `.${eventoElement1}`,
                    );
                    //console.log(eventoElement, "eventoelement1", eventoElement1);
                    if (eventoElement) {
                        // Asignar el estilo con 'important'
                        eventoElement.style.setProperty(
                            "border-left",
                            `4px solid ${colorBordeNewReservCalendar}`,
                            "important",
                        );
                    ...
                    if (
                        ...
                        document.querySelector(".services_serviceInfo_iDMQw")
                    ) {
...
function ponerHoraInicioActual() {
    ...
    let divVisualizador = document.querySelector(
        ".slotHorasCobrarServicioCalendar",
    );
    ...
    document
        ...
        .setAttribute("data-hourreserv", horaFormateada);
    marcarHoraSeleccionada(".contenedorHorasInicioCalendar", horaFormateada);

    // Hacer scroll a la hora activa en ambos contenedores
    const contenedor2 = document.querySelector(
        ".contenedorHorasInicioCalendar .scrollable",
    );
...
function ponerHoraActualMasTreinta() {
    ...
    // Hacer scroll a la hora activa en ambos contenedores
    const contenedor = document.querySelector(
        ".contenedorHorasFinCalendar .scrollable",
    );
...
function initDatePikerNewReservCalendar() {
    ...
    let fechaFlatpickrDiv2 = document.querySelector(
        ".fechaCitaInfoNewReservCalendar",
    );
    ...
    // Asignar el atributo data-piker con el formato deseado al inicio
    document
        .querySelector(".fechaCitaInfo22")
        .setAttribute("data-datepiker", fechaActual.toString());
    ...
    document
        .querySelector(".fechaCitaInfo22")
        .setAttribute("data-date", fechaFormateada2);
    ...
    flatpickr(fechaCitaInfo2, {
        ...
        onChange: function (selectedDates, dateStr, instance) {
            ...
            //estamos en la pantalla donde hay tarjetas drag para cambio fecha new reserv Calendar
            let existenTarjetasDrag = document.querySelectorAll(
                ".nuevasTarjetasMostrarOcultarCalendar .subboking-drag-el",
            );
            if (existenTarjetasDrag.length > 0) {
...
function cambiarFechaEvento(eventId, nuevaFechaInicio, nuevaFechaFin) {
    ...
    if (evento) {
        evento.setProp(
            "classNames",
            (evento.classNames || []).concat("temporal"),
        );
...
function cambiarFechaEventosArray(fecha) {
    ...
    eventosIds.forEach(function (eventId, index) {
        ...
        if (event) {
            let start = formatFechaConHora(
                fecha,
                servicesWithTimes[index].horaInicio,
            ); // Fecha y hora de inicio
            let end = formatFechaConHora(
                fecha,
                servicesWithTimes[index].horaFin,
...
function mostrarServicio(data, horaFin, contenedor) {
    ...
    document.querySelector(
        ".allservicesVistaOtroServicioCalendar",
    ).style.display = "none";
}
...
function clicTarjetasVerdesServiciosCalendar() {
    const targetServicesAddCalendar = document.querySelectorAll(
        ".services-list_serviceVariant_i9qZrAdd_calendar",
    );
    if (targetServicesAddCalendar.length) {
        targetServicesAddCalendar.forEach((targetAddCale) => {
            targetAddCale.addEventListener("click", function (event) {
                ...
                const id_serviceChange = targetAddCale.getAttribute(
                    "data-serviciochange",
                );
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if (data.encontrado) {
                            ...
                            let offcanvasInfoNewReserv =
                                document.getElementById("newReservCalendar");
                            let soloUnServicio =
                                offcanvasInfoNewReserv.querySelector(
                                    ".addServiceCalendar66",
                                );
                            let ningunServicio =
                                offcanvasInfoNewReserv.querySelector(
                                    ".addServiceCalendar",
                                );
                            let hayEmpleadoSeleccionado =
                                comprobarEmpleadoSelect(
                                    ".slotEmpleadoAddInicioCalendarAdd",
                                    "Selecciona empleado",
                                );
                            ...
                            if (
                                ...
                                const horaInicioAddCalendar =
                                    document.querySelector(
                                        ".slotHorasCobrarServicioAddCalendar",
                                    ).textContent;
                                horaFin = calcularHoraFin(
                                    horaInicioAddCalendar,
                                    data.servicio.duration.toString(),
                                );
                                const horaFinDate = new Date(
                                    "1970-01-01T" + horaFin + ":00",
                                );

                                if (confirmarExcesoDeHora(horaFinDate)) {
                                    const confirmarHoraExcedeCierre = confirm(
                                        "!!Atención el servicio excede la hora de cierre, ¿Deseas continuar?",
                                    );
                                ...
                                if (hayEmpleadoSeleccionado) {
                                    ...
                                    if (botonEditarServicioReserva) {
                                        ...
                                        console.log(
                                            "Editar reserva en nueva reserva",
                                        );
                                    } else {
                                        console.log(
                                            "no hay boton editar es añadir servicio en nueva reserva",
                                        );
                                    }
                                    console.log(
                                        "hay empleado actualizar evento idEvento:",
                                        eventIdChangeCalendar,
                                        data.servicio.borderColor,
                                    );
                                    ...
                                    // me he quedado aquí por eso cambia el color del borde
                            ...
                            if (
                                ...
                                if (
                                    document
                                        .querySelector(
                                            ".allServicesAddCalendar00",
                                        )
                                        .classList.contains(
                                            "allservicesModificarInicialCalendar",
                                        )
                                ) {
                                    contenedor = ".addServiceCalendar66";
                                    const id_serviceOld = document
                                        .querySelector(
                                            ".services_serviceInfo_iDMQwAddCalendar",
                                        )
                                        .getAttribute("data-indexActual");
                                ...
                                const horaInicio00 = document
                                    .querySelector(
                                        ".slotHorasCobrarServicioCalendar",
                                    )
                                    .getAttribute("data-hourreserv");
                                const horafin00 = calculateEndTime(
                                    horaInicio00,
                                    data.servicio.duration,
                                );
                                ...
                                // Hacer scroll a la hora activa en ambos contenedores
                                const contenedor33 = document.querySelector(
                                    ".contenedorHorasFinCalendar .scrollable",
                                );
                                //función para hacer scroll hora inicio y fin activas
                                function intentarScroll(contenedor) {
                            ...
                            const hayEmpleadoSeleccionado2 =
                                comprobarEmpleadoSelect(
                                    ".slotEmpleadoAddInicioCalendarAdd",
                                    "Selecciona empleado",
                                );
...
function actualizarEvento(
    ...
    if (eventoTemporal) {
        eventoTemporal.style.setProperty(
            "border-left",
            `4px solid ${colorBorde}`,
            "important",
        );
        eventoTemporal.style.setProperty(
            "border-top",
            `1px solid ${colorBorde}`,
            "important",
        );
    }
    ...
    if (evento) {
        ...
        if (botonEditarServicioReserva) {
            idEventoUtilizar =
                botonEditarServicioReserva.getAttribute("data-id-unico");
            eventEl = document.querySelector(
                `a[data-idunicoevento="${idEventoUtilizar}"]`,
            );
...
function actualizarEventoClienteArray(cliente) {
    ...
    eventosFiltrados = eventos.filter((evento) =>
        evento.id.startsWith("temporalArray_"),
    );
    ...
    if (infoArrayEnvio.length) {
        ...
        eventosFiltrados = eventos.filter((evento) =>
            evento.id.startsWith("eventoTemporal_"),
        );
...
function botonGuardarCalendar() {
    const botonGuardarServicio = document.getElementById(
        "uid-377-inputCalendar",
    );
    if (botonGuardarServicio) {
        botonGuardarServicio.addEventListener("click", function () {
            if (fechaEventoIdUnico) {
                ...
                fechaEventoIdUnico = formatearFeAnio(
                    document
                        .getElementById("datePikerfechaCitaInfo22")
                        .getAttribute("data-datepiker"),
                );
            ...
            document
                .getElementById("uid-377-inputCalendar")
                .classList.add("index_is--disabled_w97Nq");

            let id_servicio0120 = document
                .querySelector(".selectServiceAddCalendar")
                .getAttribute("data-service");
            addServiceArray(parseInt(id_servicio0120, 10));

            let element33 = document.querySelector(
                ".selectServiceAddCalendar .services_serviceDecorator_ldMxA",
            );
            ...
            colorBordeReservArray.push(colorborde);
            let horaIncioAddCalendar = document.querySelector(
                ".slotHorasCobrarServicioAddCalendar",
            ).textContent;
            let horaFinAddCalendar = document.querySelector(
                ".slotHoraFinCorbrarServicioAddCalendar",
            ).textContent;
            let duracion = calcularDuracion(
                horaIncioAddCalendar,
                horaFinAddCalendar,
            );
            ...
            document.querySelector(".newReservCalendar00").style.display =
                "block";
            let divTarjetasInicialesCalendar = document.querySelector(
                ".tarjetasIncialesMostrarOcultarCalendar",
            );
            divTarjetasInicialesCalendar.classList.add("d-none");

            let divNuevasTrajetasCalendar = document.querySelector(
                ".nuevasTarjetasMostrarOcultarCalendar",
            );
            getServicesById(selectedServiceIds2, function (servicios) {
                ...
                getAllEmpleados(function (empleadosReservas) {
                    ...
                    let eventosTemporales = eventos.filter((event) =>
                        event.id.startsWith("eventoTemporal_"),
                    );
...
function meterHorasArrayVistaCalendar() {
    let horaIncioScreenCalendar = document.querySelector(
        ".slotHorasCobrarServicioCalendar",
    ).textContent;
    let horaFinScreenCalendar = document.querySelector(
        ".slotHoraFinCorbrarServicioCalendar",
    ).textContent;
    let idEmpleadaCalendar = document
        .querySelector(".slotEmpleadoAddInicioCalendar")
        .getAttribute("data-empleid");
    selectedServiceIds.forEach((serviceId) => {
        servicesWithTimes.push({
            id_unico:
                ...
                serviceId +
                "_" +
                horaIncioScreenCalendar.replace(":", "_") +
                "__" +
                horaFinScreenCalendar.replace(":", "_") +
                "_" +
                idEmpleadaCalendar,
            id: serviceId,
...
function clicWhiteTargeAddServiceCalendar() {
    const tarjetaAniadirServicio = document.querySelector(
        ".addServiceCalendar",
    );
...
function clicTarjetaModificarServicioIncialCalendar() {
    const tarjetaSecundarioCalendar = document.querySelector(
        ".addServiceCalendar66",
    );
    if (tarjetaSecundarioCalendar) {
        tarjetaSecundarioCalendar.addEventListener("click", function () {
            ...
            document
                .querySelector(".allServicesAddCalendar00")
                .classList.add("allservicesModificarInicialCalendar");
        });
    }
}
...
function clicTarjetaSeleccionarOtroServicioCalendar() {
    let botonAddServiceAddCalendar = document.querySelector(
        ".selectServiceAddCalendar",
    );
    if (botonAddServiceAddCalendar) {
        botonAddServiceAddCalendar.addEventListener("click", function () {
            document
                .querySelector(".allServicesAddCalendar00")
                .classList.add("allservicesVistaOtroServicioCalendar");
            openAllservicesPlantillaCalendar();
        });
    }
}
...
function clicBotonAniaridServicioCalendar() {
    ...
    document.querySelector(".buttonEditStrackModifyNewReserv").style.display =
        "none";
    document.querySelector(".buttonSavetrackModifyNewReserv").style.display =
        "flex";
    ...
    let seleccionaCliente_valor =
        document.getElementById("solicictaCliente").value;
    let horaInicioDuracion = document.querySelector(
        ".slotHorasCobrarServicioCalendar",
    ).textContent;
    let horaFinDuracion = document.querySelector(
        ".slotHoraFinCorbrarServicioCalendar",
    ).textContent;
    ...
    if (tarjetasServiciosMultiples.length === 0) {
        ...
        let fechaIdUnico = formatearFeAnio(
            document
                .getElementById("datePikerfechaCitaInfo22")
                .getAttribute("data-datepiker"),
        );
        ...
        let element = document.querySelector(
            ".addServiceCalendar66 .services_serviceDecorator_ldMxA",
        );
        if (element) {
    ...
    let totalServicios = selectedServiceIds2.length + 1;
    let divTotalServicios = document.querySelector(
        ".totalServiciosAñadidosCalendar",
    );
    if (!divTotalServicios) {
        divTotalServicios = document.querySelector(
            ".allServicesAddCalendar00Add .totalServiciosAñadidos",
        );
    }
    divTotalServicios.textContent = `Servicio #${totalServicios}`;
    ...
    let divHoraInicioAddCalendar = document.querySelector(
        ".slotHorasCobrarServicioAddCalendar",
    );
    if (servicesWithTimes.length >= 2) {
        //obtener última posicion array
        ...
        //cojer hora fin pantalla anterior y ponerla de inicio allServicesAddCalendar00Add
        horaInicioAddCalendar = document.querySelector(
            ".slotHoraFinCorbrarServicioCalendar",
        ).textContent;
    }
    ...
    document.querySelector(
        ".slotHoraFinCorbrarServicioAddCalendar",
    ).textContent = calcularHoraFin(horaInicioAddCalendar, "30min");
    divHoraInicioAddCalendar.textContent = horaInicioAddCalendar;
    //poner el valor al input hora inicio
    document.getElementById("horaNewServiceInputInicioAddCalendar").value =
        horaInicioAddCalendar;
    //poner el valor al input hora fin
    document.getElementById("horaNewServiceInputFinAddCalendar").value =
        calcularHoraFin(horaInicioAddCalendar, "30min");
    //marcar hora inicio
    marcarHoraSeleccionada(
        ".contenedorHorasInicioAddCalendar",
        horaInicioAddCalendar,
    ...
    // Hacer scroll a la hora activa en ambos contenedores
    const contenedor444 = document.querySelector(
        ".contenedorHorasInicioAddCalendar .scrollable",
    );
    const contenedor222 = document.querySelector(
        ".contenedorHorasFinAddCalendar .scrollable",
    );
    ...
    let corazonCalendarPantalla2 = document.querySelector(
        ".solicitadoClientePantalla2 ",
    );
    corazonCalendarPantalla2.src =
        urlAplicacion + "/storage/calendar/heart-empty.svg";
...
function resetPantalla2NewReservCalendar() {
    ...
    if (botonEditarServicioReserva) {
        ...
        let nombreServicio = botonEditarServicioReserva
            .getAttribute("data-name-service")
            .trim();
        let borderColor = botonEditarServicioReserva
            .getAttribute("data-border")
            .trim();
        borderColor = borderColor.trim();
        ...
        let noHayServicio = document.querySelector(
            ".selectServiceAddCalendar .services-wrapper_serviceEmpty_pbusk",
        );
        let contenidoSlotNombreEmpleado = document
            .querySelector(".slotEmpleadoAddInicioCalendarAdd")
            .textContent.trim();
        let hayEmpleadoSeleccionado =
            contenidoSlotNombreEmpleado !== "Selecciona empleado";
    ...
    document
        .querySelector(".contenedorHorasFinAddCalendarclass")
...
function clicCruzNewReserCalendar() {
    ...
    let vistaAddNewService = document.querySelector(
        ".allServicesAddCalendar00Add",
    );
    ...
    document
        .querySelector(".contenedorHorasFinCalendarclass")
        .classList.remove("border-warning2");
...
function clicFlechaAtrasVistaAniadirServicio() {
    const flecha = document.querySelector(
        ".closedOffcanvasNewReservCalendar00Add",
    );
    if (flecha) {
...
function showAllNewReservCalendarPlantilla() {
    fetch("show-newReservCalendar-plantilla")
        ...
        .then((data) => {
            ...
            document.querySelector(
                ".contenedorOfcanvasNewReservCalendar",
            ).innerHTML = data;
            clicWhiteTargeAddServiceCalendar();
            clicTarjetasVerdesServiciosCalendar();
            ...
            manejarClicPestania(
                ".cita_tabNewReserv",
                "datos_reservaNewReserv0106",
            );
            manejarClicPestania(
                ".nota_tabNewReserv",
                "notas_reservaNewReserv0106",
            );
            // clicFlechaAtrasPantalla2NewReservCalendar();
            manejarEstilosTexareaLabel(
                ".business_secret_noteNewReserv",
                "business_secret_noteNewReserv",
            );
            manejarEstilosTexareaLabel(
                ".business_noteNewReservInfo",
                "business_noteNewReservInfo",
            );
            manejarEstilosTexareaLabel(
                ".business_noteInfo",
...
function scrollToHour(targetHour) {
    ...
    const scrollGridBody = document.querySelector(
...
function eliminarEventosTemporales(claseEventoTemporal) {
    ...
    const eventosTemporales = eventos.filter((event) =>
        event.id.startsWith(claseEventoTemporal),
    );
    eventosTemporales.forEach((evento) => {
        evento.remove();
...
function eliminarEventosTemporalesByClass(claseEventoTemporal) {
    ...
    const eventosTemporales = eventos.filter((event) => {
        ...
        return Array.from(clasesEvento).some((clase) =>
            clase.startsWith(claseEventoTemporal),
        );
    });
...
function obtenerDatosGeneralesGuardarReserva(totalPagarClass, idDatePiker) {
    ...
    let dateTime1 = document
        .getElementById(idDatePiker)
        .getAttribute("data-datepiker");

    // Obtener las notas internas y el mensaje para el cliente
    let notaInternaCalendar =
        document.getElementById("business_secret_noteNewReserv").value.trim() ||
        null;
    let mensaje_for_client =
        document.getElementById("business_noteNewReservInfo").value.trim() ||
        null;

    // Obtener el total a pagar
    let total_pagarReservaNewCalendar = parseFloat(
        document.querySelector(totalPagarClass).textContent,
    );
...
//genera los datos para usar en la función guardarReservaNewReservCalendar
function obtenerDatosGeneralesGuardarReserva2(
    ...
    let durationCalendar = calcularDuracion(
        servicesWithTimes[0].horaInicio,
        servicesWithTimes[0].horaFin,
    );
    let dateTimeCalendar = formatearFecha4(
        datosReservaGeneral,
        servicesWithTimes[0].horaInicio,
    ); //2024-12-13 09:00:00
...
function gurardarReservaNewReservCalendar() {
    let botonGuardarNuevaReservaCalendar =
        document.getElementById("uid-798-input");
    if (botonGuardarNuevaReservaCalendar) {
        botonGuardarNuevaReservaCalendar.addEventListener(
            "click",
            function (event) {
                event.preventDefault();
                ...
                let datosReservaGeneral = obtenerDatosGeneralesGuardarReserva(
                    ".totalPagarNewReservCalendar",
                    "datePikerfechaCitaInfo22",
                );
                let crear_reserva = "";

                getConfiguracionReservas(function (configuraciones) {
                    let confirmacionAutomaticaCalendar =
                        configuraciones[0].confirmacion_automatica;
                    confirmacionAutomaticaCalendar =
                        confirmacionAutomaticaCalendar === "si"
                            ? "confirmed"
                            : "pending";
                    ...
                    if (servicesWithTimes.length === 0) {
                        ...
                        let fecha = formatearFeAnio(
                            datosReservaGeneral.dateTime1,
                        );

                        meterHorasArrayInicioNoTimeOut(
                            ".slotHorasCobrarServicioCalendar",
                            ".slotHoraFinCorbrarServicioCalendar",
                            ".slotEmpleadoAddInicioCalendar",
                            datosReservaGeneral.seleccionaCliente_valor,
                            fecha,
                        );
                        let datosReservaGeneral2 =
                            obtenerDatosGeneralesGuardarReserva2(
                                confirmacionAutomaticaCalendar,
                                datosReservaGeneral.dateTime1,
                            );
                        ...
                        let start = formatFechaConHora(
                            datosReservaGeneral.dateTime1,
                            datosReservaGeneral2.horaInicio,
                        ); // Fecha y hora de inicio
                        let fechaInicial = start.split("T")[0];
                        ...
                        $.ajax({
                            ...
                            data: {
                                ...
                                date_time:
                                    datosReservaGeneral2.dateTimeCalendar,
                                ...
                                total_payment:
                                    datosReservaGeneral.total_pagarReservaNewCalendar,
                                empleado_seleccionado:
                                    servicesWithTimes[0].seleccionaCliente,
                                nota_interna:
                                    datosReservaGeneral.notaInternaCalendar,
                                mensaje_cliente:
                                    datosReservaGeneral.mensaje_for_client,
                            ...
                            success: function (response) {
                                ...
                                if (reserva_creada === true) {
                                    ...
                                    calendar.gotoDate(fechaInicial);
                                    desactivarLoaderUniversal(
                                        "loaderSperaAdministrator",
                                    );
                                    insertMessageResolAction(
                                        "Reserva creada con éxito",
                                        "#Citas_administrator",
                                        datosReservaGeneral.stylos,
                                        "ok",
                                    );
                                } else {
                                    alert(
                                        "Atención!! la hora seleccionada o el empleado no están disponibles, inténtelo de nuevo cambiando esos datos",
                                    );
                                    desactivarLoaderUniversal(
                                        "loaderSperaAdministrator",
                                    );
                        ...
                        servicesWithTimes.forEach((service) => {
                            service.date_time = formatearFecha4(
                                datosReservaGeneral.dateTime1,
                                service.horaInicio,
                            );
                        });
                        ...
                        $.ajax({
                            ...
                            data: {
                                _token: datosReservaGeneral.csrfToken,
                                arrayCompleto:
                                    JSON.stringify(servicesWithTimes),
                            ...
                            success: function (response) {
                                ...
                                if (reserva_creada === true) {
                                    ...
                                    insertMessageResolAction(
                                        "Reserva múltiple creada con éxito",
                                        "#Citas_administrator",
                                        datosReservaGeneral.stylos,
                                        "ok",
                                    );
...
function getReservaMasTarde(reservas) {
    ...
    reservas.forEach((reserva) => {
        ...
        const horaFin = new Date(
            fechaInicio.getTime() + reserva.duration * 60000,
        ); // 60000 ms = 1 minuto
...
function actualizarIndicadorNota(textareaIds, spanClass) {
    ...
    const tieneTexto = textareas.some(
...
function actualizarIndicadorNotaTodosInputs() {
    ...
    if (document.getElementById("business_secret_noteNewReserv")) {
        document
            .getElementById("business_secret_noteNewReserv")
            .addEventListener("input", () => {
                actualizarIndicadorNota(
                    ...
                    ".indicatorNotasNewReserv",
                );
            });
    }
    if (document.getElementById("business_noteNewReservInfo")) {
        document
            .getElementById("business_noteNewReservInfo")
            .addEventListener("input", () => {
                actualizarIndicadorNota(
                    [
                        "business_secret_noteNewReserv",
    ...
    if (document.getElementById("business_noteNewReserv")) {
        document
            .getElementById("business_noteNewReserv")
            .addEventListener("input", () => {
                actualizarIndicadorNota(
                    ["business_noteNewReserv", "business_noteInfo"],
                    ".indicatorNotasNewReservInfo",
                );
            });
    }
    if (document.getElementById("business_noteInfo")) {
        document
            .getElementById("business_noteInfo")
            .addEventListener("input", () => {
                actualizarIndicadorNota(
                    ["business_noteNewReserv", "business_noteInfo"],
                    ".indicatorNotasNewReservInfo",
...
function blockPointerEvents() {
    ...
    let calendarioABloquear = document.querySelector(
        ".fc-resourceTimeGridDay-view tbody",
    );
...
function showViewCobrarServicioCalendar() {
    ...
    let id_reserva = document
        .getElementById("eventDetailsModal")
        .getAttribute("data-idreserv");
    let divContenedorContenedores = document.querySelector(
        ".wrapper_tabcontent",
    );
    let contenedoresActivos =
        divContenedorContenedores.querySelectorAll(".active");
    ...
    if (infoArrayEnvio.length) {
        ...
        if (infoArrayEnvio[0].extendedProps.usuario.id) {
            insertarTarjetaClienteSelecionadoCobrarCalendar(
                infoArrayEnvio[0].extendedProps.usuario,
                ".basket-customer-card0101",
                "card_empty_ventas",
            );
        ...
        insertDateServiceByCobrarCalendar(
            id_servicio,
            nombreEmpleado,
            infoArrayEnvio.event.extendedProps.empleada.id,
            infoArrayEnvio.event.extendedProps.servicio.precio,
            usuario,
        );
        if (usuario.id) {
            insertarTarjetaClienteSelecionadoCobrarCalendar(
        ...
        document
            .querySelector(".basket-layout_sidebar_X6qEm")
            .setAttribute("data-idReserv", id_reserva);
    }
    ...
    activarPestaniaNuevaVenta();
    showDivPestaniasLateralesVentaRapida(
        "divContenedorVentaRapidaBasket-menu-quick-sale",
    );
    reiniciarPestaniaVentaRapida();
...
function activarPestaniaNuevaVenta() {
    quitarActiveListaNavegacionPrincipalVentaRapida();
    let pestañaNuevaVenta = document.querySelector(
        'li[data-testid="checkout"]',
    );
...
function showDivPestaniasLateralesVentaRapida(selectedDivId) {
    const divs = [
        document.querySelector(
            ".divContenedorVentaRapida.basket-menu-quick-sale",
        ),
        document.querySelector(
...
$(document).ready(function () {
    $('[data-testid="b-custom-header-icon-back_transaction"]').on(
        "click",
        function () {
            showDivVentaRapida("index_sales_X5DVI");
            activarPestaniaNuevaVenta();
...
function insertDateServiceByCobrarCalendar(
    servicio_id,
    nombreEmpleado,
    id_empleado026,
    precioServicio,
...
// insertar tarjeta cliente seleccionado venta rápida
function insertarTarjetaClienteSelecionadoCobrarCalendar(
    cliente,
    divInfoClienteEnviado,
    claseDiferenciadora,
) {
...
function insertTarjetaServiciosCobrarCalendar() {
    ...
    document.querySelector(".statusReservaCalendarCobrar").textContent =
        statusCobrar;
    let divContendorServicios = document.querySelector(
        ".tarjetasServiciosCobrarCalendar056",
    );
...
let botonBorrarCobrarCalendar = document.querySelector(
    ".botonCerrarTarjeta028",
);
    ...
    const renderMode = (mode) => {
        ...
        switch (mode) {
                    ...
                    onChange: function (selectedDates) {
                        if (selectedDates.length > 0) {
                            ...
                            document.getElementById(
                                ...
                                `Semana ${weekNumber} (${formatFullDate(first)} - ${formatFullDate(last)})`;
                        }
                    },
                });
                document.querySelector(".flatpickr-weekday").textContent =
                    "Semana";
                break;
            ...
            case "year":
                ...
                document
                    .querySelectorAll(".b-scopepicker-button")
                    .forEach((div) => {
...
async function cargarPagosFechaMasReciente(
    contenedorId = "detalleTransacciones22",
) {
    ...
    try {
        ...
        const pagosRecientes = data.filter((pago) => {
            const fechaPagoStr = new Date(pago.fecha)
                .toISOString()
                .split("T")[0];
        ...
        pagosRecientes.forEach((payment) => {
            ...
            const metodoRaw = quitarAcentos(
                payment.metodo_pago.toLowerCase(),
            ).replace(/\s+/g, "_");
...
async function cargarPagosPorFechaExacta(
    fechaExacta,
    contenedorId = "detalleTransacciones22",
) {
    const contenedor = document.getElementById(contenedorId);
...
function cerrarModalDetallesRecibo() {
    let modalRecibo = document.querySelector(
        `.modalMotrarReciboDetalleTransaccion`,
    );
    // console.log("Hola");
...
async function mostrarDetallesRecibo(reciboId) {
    ...
    try {
        ...
        let contenedorDeTiketModal = document.querySelector(
            ".receipt_receiptReceipt_KzM2ZModalMostrarRecibo",
        );
        ...
        $("#uid-340-inputEnviarEmail").on("click", function () {
            ...
            $.ajax({
                ...
                data: {
                    ...
                    valor_neto: data2.recibo.valor_neto
                        .toString()
                        .replace(".", ","),
                    importe_iva: data2.recibo.importe_impuesto
                        .toString()
                        .replace(".", ","),
                    valor_bruto: data2.recibo.valor_bruto,
                    subtotal: data2.recibo.subtotal,
                    descuentoTotalPorcentaje:
                        data2.recibo.descuento_total_porcentaje,
        ...
        console.error("Error al cargar pagos", error);
        contenedor.innerHTML =
            "<li>Error al cargar los detalles de los servicios.</li>";
    }
...
function reiniciarPestaniaVentaRapida() {
    ...
    let elementosliDiv = document.querySelectorAll(
        ".pestanias_laterales_ventaRapida li div",
    );
    ...
    const element = document.querySelector(
        'div[data-testid="basket-menu-quick-sale"]',
    );
    if (element) {
    ...
    document
        .querySelectorAll(".divContenedorVentaRapida")
        .forEach((contentDiv) => {
...
document
    .querySelectorAll(".pestanias_laterales_ventaRapida li")
    .forEach((li) => {
        li.onclick = function () {
            ...
            if (
                divDentroDeLi.classList.contains("index_menuItemActive_bAiPX")
            ) {
            ...
            // Mostrar el div correspondiente
            const matchingDiv = document.querySelector(
                `.divContenedorVentaRapida.${dataTestId}`,
            );
            if (matchingDiv) {
                matchingDiv.classList.remove("d-none");
            }
...
async function reservasFinalizadasPendientesPago() {
    try {
        ...
        reservasAgrupadasArray = agruparReservasPorMultiple(
            reservasAgrupadasArray,
        );
        ...
        let contenedorReservasHoy = document.querySelector(
            ".booking-notPay_end_hoy",
        );
        ...
        let contenedorReservasAyer = document.querySelector(
            ".booking-notPay_end_ayer",
        );
        ...
        let contenedorReservasAntiguas = document.querySelector(
            ".booking-notPay_end_resto",
        );
        ...
        if (
            ...
            $(contenedorReservasAntiguas).empty();
            document
                .querySelector(".booking-notPay_end_all33")
                .classList.add("d-none");
            document
                .querySelector(".noHayTransaccionesParaCobrar")
                .classList.remove("d-none");
        } else {
            document
                .querySelector(".booking-notPay_end_all33")
...
function cargarDatosReservaNoPay(id_reserva, elementoClicado) {
    ...
    for (let reserva of reservasAgrupadasArray) {
        if (
            reserva.tipo === "simple" &&
            reserva.reservas[0].id === id_reserva
        ) {
            reservaSeleccionada = reserva;
            break;
        } else if (
            reserva.tipo === "multiple" &&
            reserva.multiple_id === id_reserva
        ) {
            reservaSeleccionada = reserva;
            break;
        }
...
async function insertDateServiceNotPay() {
    ...
    document
        .querySelector(".basket-layout_sidebar_X6qEm")
        .setAttribute("data-idReserv", id_reservaParaAtributo);
    ...
    if (reservaSeleccionadaNoPay.usuario) {
        insertarTarjetaClienteSelecionado(
            reservaSeleccionadaNoPay.usuario,
            ".basket-customer-card0101",
            "card_empty_ventas",
        );
    }
    ...
    const fetchEmpleadoById = (empleadoId) => {
        return fetch("get-empleadoById", {
            ...
            if (!response.ok)
                throw new Error("Error al obtener datos del nuevo empleado");
    ...
    reservaSeleccionadaNoPay.reservas.forEach((reserva) => {
        const li = Array.from(liElements).find(
            (el) => el.getAttribute("data-li-index") == reserva.id,
        );
        if (!li) return;
        ...
        if (reserva.empleada && reserva.empleada.id != nuevoEmpleadoId) {
            ...
            const update = fetchEmpleadoById(nuevoEmpleadoId)
                ...
                .catch((error) => {
                    console.error(
                        `Error al actualizar empleado para reserva ${reserva.id}:`,
                        error,
                    );
                });
...
if (document.getElementById("loaderVentaRapidaLiveWire")) {
    setTimeout(() => {
        document
            .getElementById("loaderVentaRapidaLiveWire")
            .classList.add("d-none");
    }, 4000);
}
//pestañas
...
items.forEach((item) => {
    item.addEventListener("click", function () {
        ...
        activarLoaderUniversal("loaderVentaRapida");
        items.forEach((el) =>
            el.classList.remove("b-tabs_tabBorderedActive_ff9lg"),
        );
...
if (document.getElementById("uid-771-input-infoReserv")) {
    document
        .getElementById("uid-771-input-infoReserv")
        .addEventListener("click", function () {
            document.getElementById(
                "modalDescartarCambiosInfoReserv",
            ).style.display = "flex";
        });
}
...
if (document.getElementById("uid-772-input-infoReserv")) {
    document
        .getElementById("uid-772-input-infoReserv")
        .addEventListener("click", function (event) {
            event.preventDefault();
            // console.log("clic en guardar modificación");
            document.getElementById(
                "modalSeguroGuardarCambiosInfoReserv",
            ).style.display = "flex";
        });
}
...
if (document.getElementById("uid-2299-input_siGuadarCambios")) {
    document
        .getElementById("uid-2299-input_siGuadarCambios")
        .addEventListener("click", function () {
            // general
            activarLoaderUniversal("loaderSperaAdministrator");
            document.getElementById(
            ...
            getConfiguracionReservas(function (configuraciones) {
                ...
                if (
                    ...
                    selectedServiceIds = [];
                    let idServicio = document
                        .querySelector(".getOldService")
                        .getAttribute("data-idserviceadd");
                    ...
                    $.ajax({
                        ...
                        success: function (response) {
                            const reserva_actualizada =
                                response.reservaActualizada;
                    ...
                    $.ajax({
                        ...
                        success: function (response) {
                            ...
                            if (reserva_creada === true) {
                                ...
                                insertMessageResolAction(
                                    "Reserva múltiple actualizada con éxito",
                                    "#Citas_administrator",
                                    datosReservaGeneral.stylos,
                                    "ok",
                                );
...
if (document.getElementById("uid-2300-input_noGuardarCambios")) {
    document
        .getElementById("uid-2300-input_noGuardarCambios")
        .addEventListener("click", function () {
            document.getElementById(
                "modalSeguroGuardarCambiosInfoReserv",
            ).style.display = "none";
...
async function actualizarCitasProxiPasada() {
    try {
        ...
        const contenedorProximas = document.getElementById(
            "citasProcimasContainer",
        );
        const contenedorPasadas = document.getElementById(
            "citasTerminadasContainer",
        );
...
function formatearFechaCita(dateString) {
    ...
    return {
        mesYAnio: fecha.toLocaleDateString("es-ES", {
            month: "short",
            year: "numeric",
        }),
        dia: fecha.getDate(),
...
function renderPagoHTML(citaProxima) {
    ...
    const precio =
        citaProxima.numero_servicios > 1
...
function renderEstadoCitaHTML(citaProxima) {
    ...
    if (citaProxima.cliente_confirmo_modificacion === "confirmado") {
        ...
        if (
            citaProxima.status === "pending" ||
            citaProxima.status === "Finalizada"
        ) {
            clases =
                "purify_Ks8Q8dHEaaaFeDYdNtADtw== purify_YDeoXrcjLlEdNmPC-e55Hw== purify_my7kvaO77EFZtdkGbwjvyA== purify_kq4BZr36QXoLgkAnN95TWw==";
        } else if (citaProxima.status === "cancelled") {
            clases =
                "purify_Ks8Q8dHEaaaFeDYdNtADtw== purify_Mq+0nEBBXOo2GlJ3m+Dovg== purify_my7kvaO77EFZtdkGbwjvyA== purify_kq4BZr36QXoLgkAnN95TWw==";
        } else if (citaProxima.status === "no_asistida") {
            clases =
                "bg-warning text-black purify_Ks8Q8dHEaaaFeDYdNtADtw== purify_D1nq2s-huXm2lObUUwFyqA== purify_my7kvaO77EFZtdkGbwjvyA== purify_kq4BZr36QXoLgkAnN95TWw==";
        } else if (citaProxima.status === "confirmed") {
            clases =
                "purify_Ks8Q8dHEaaaFeDYdNtADtw== purify_D1nq2s-huXm2lObUUwFyqA== purify_my7kvaO77EFZtdkGbwjvyA== purify_kq4BZr36QXoLgkAnN95TWw==";
        }
...
if (botonPuntitos1) {
    botonPuntitos1.addEventListener("click", function () {
        ...
        document
            .querySelector(".bocadilloDroponModificarRecibo")
    ...
    switch (id) {
        case "aceptarCamBioCita":
            ...
            if (document.getElementById("offcanvasCita")) {
                idReserva = document
                    .getElementById("offcanvasCita")
                    .getAttribute("data-citaid");
            }
            // console.log(idReserva, "idReserva");
            const response = await fetch(
                "Reservas/confirmar-modificacion/" + idReserva,
                {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
        ...
        case "cancelarCambioCita":
            ...
            const response2 = await fetch(
                "Reservas/obtenerReservas/" + idReserva2,
                {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
            ...
            if (data2.success) {
                ...
                var myModal = new bootstrap.Modal(
                    document.getElementById("modalPreguntaCancelarCita"),
                );
                myModal.show();
                mostrarServicios(data2.reservas);

                //mostrar fecha
                const spanFecha = document.querySelector(
                    '[data-testid="appointment-info-time"]',
                );
                ...
                spanFecha.textContent = formatearFechaModalCancelar(
                    primeraReserva.date_time,
                );
                //poner datos botón reprogramar fecha
                let contenedorbotones = document.querySelector(
                    ".contenedorBotonesPreguntarCancelRepro",
                );
...
async function montarOffcanvasPregunta() {
    ...
    let reservas1 = JSON.parse(
        $(".reprogramarCitaPregunta").attr("data-reservas"),
    );
    ...
    if (document.getElementById("modalServiciosSeleccionados2")) {
        ...
        let modal = bootstrap.Modal.getInstance(
            document.getElementById("modalServiciosSeleccionados2"),
        );
        if (modal) modal.hide();
    } else {
    ...
    let dias = document.querySelectorAll(
        `#carousel_dias${index33} .carrusel-inner-dias .dia`,
    );
    ...
    let promesas = diasVisibles.map(async (dia) => {
        ...
        try {
            let horasRecibidas = await obtenerHorasNoDiaActivo(
                fecha55,
                index33,
                duration,
            );
            let porcentaje = calcularPorcentajeDisponibilidad(
                horasRecibidas,
                parseInt(duration),
                diaSemana,
            );
    ...
    manejarSeleccionFechaMultipleServicio(
        fechaSeleccionada,
        index33,
        duration,
        horaInicio,
    ...
    let offcanbasBottomReservaPregunta = document.getElementById(
        `offcanvasBottomReserva${index33}`,
    );
    let contenedorServicios = offcanbasBottomReservaPregunta.querySelector(
        ".serviciosMultiples",
    );
    ...
    offcanbasBottomReservaPregunta.querySelector(
        ".totalPagarReservaOffcanva",
    ).textContent = `${Number(totalPrecioPagar).toFixed(2)} €`;
    offcanbasBottomReservaPregunta.querySelector(
        ".tiempoTotalReservOffcanva",
...
function mostrarServicios(reservas) {
    const spanServicios = document.querySelector(
        '[data-testid="appointment-info-services"]',
    );

    const textoServicios = reservas.map((r) => r.servicio.nombre).join(" + ");
    ...
    switch (id) {
        ...
        case "cancelarCitaPregunta2":
            ...
            const response3 = await fetch(
                "Reservas/cancelar-modificacion/" + idReserva3,
                {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
...
let botonElinarServicioEditService = document.getElementById(
...
if (botonElinarServicioEditService) {
    botonElinarServicioEditService.addEventListener("click", function () {
        ...
        if (document.querySelector(".selectServiceAdd")) {
            ...
            let id_serviceDelete =
                botonEditServiceModify.getAttribute("data-id-unico");
            let id_reserva =
                botonEditServiceModify.getAttribute("data-id-reserva");
            console.log(
                id_reserva,
                id_serviceDelete,
                "ID RESERVA BOTON ELIMINAR",
            );
            ...
            if (
                ...
                infoArrayEnvio = infoArrayEnvio.filter(
                    (evento) =>
                        evento.extendedProps.id_unico !== id_serviceDelete,
                );
            } else {
            ...
            let id_eventoCalendarioEliminar =
                botonEditarServicioReserva.getAttribute("id");
            calendar.getEventById(id_eventoCalendarioEliminar).remove();
            console.log(
                id_eventoCalendarioEliminar,
                "id_evento a eliminar-------------------",
            );
            ...
            if (servicesWithTimes.length > 1) {
                ...
                getServicesById(selectedServiceIds2, function (servicios) {
                    ...
                    getAllEmpleados(function (empleadosReservas) {
                        ...
                        servicesWithTimes.forEach((service) => {
                            ...
                            let empleado = empleadosReservas.find(
                                (emp) =>
                                    emp.id === parseInt(service.id_empleado),
                            );
                            if (empleado) {
                                // Obtenemos las primeras dos letras del nombre del empleado
                                let iniciales = empleado.nombre
                        ...
                        arrayIdsReservas = arrayIdsReservas.filter(
                            (id) => id !== idReservaInt,
                        );
                        let arrayIdsUnicos = servicesWithTimes.map(
                            (item) => item.id_unico,
                        );
                        ...
                        console.log(
                            ...
                            arrayIdsUnicos,
                            idsEventosParaHtml,
                            "servicesWithTimes infoArrayEnvio-----",
                        );
                        servicios.forEach((servicio, index) => {
                            let precioNumerico = parseFloat(servicio.precio);
                            totalPricePay += precioNumerico;
                            ...
                            let seleccionaCliente =
                                servicesWithTimes[index].seleccionaCliente;
                            let id_reserva =
                                servicesWithTimes[index].id_reserva;
                        ...
                        $(divNuevasTrajetasCalendar).empty();
                        $(divNuevasTrajetasCalendar).append(
                            htmlContentCalendar,
                        );
                        addHtmlDivPrecioFinal(
                            totalPricePay,
                ...
                document
                    .querySelector(".tarjetasIncialesMostrarOcultar")
                    .classList.remove("d-none");
                let divTarjetasNuevas = document.querySelector(
                    ".nuevasTarjetasMostrarOcultar",
                );
                ...
                if (extendedProps.seleccionado_cliente === 1) {
                    corazon.src =
                        urlAplicacion +
                        "/storage/calendar/corazonRojoEmpleCliente.svg";
                    document.getElementById("solicictaCliente").value = 1;
                } else {
                    corazon.src =
                        urlAplicacion + "/storage/calendar/heart-empty.svg";
                ...
                document
                    ...
                    .setAttribute(
                        "data-oldDuration",
                        extendedProps.servicio.duracion,
                    );
                document.querySelector(
                    ".services_servicePrice_wErzf",
                ).textContent = extendedProps.servicio.precio;
                document.querySelector(".slotHorasCobrarServicio").textContent =
                    formatTime(extendedProps.fecha);
                document
                    .querySelector(".slotHorasCobrarServicio")
                ...
                document.querySelector(
                    ".slotHoraFinCorbrarServicio",
                ).textContent = obtenerHoraEuropaCentral(infoArrayEnvio[0].end);
                cambiarTotales(extendedProps.servicio.precio);
                //empleado pongo nombre empleado global
                ponerNomIdEmpleInicio(
                    extendedProps.empleada.nombre,
                    extendedProps.empleada.id,
                );
                //reserva sencilla
            }
            setTimeout(() => {
...
/*
CLIENTES AÑADIR Y VER DETALLES
*/

/**
 * ABRIR VISTA CREAR CLIENTE
 */
const boton = document.querySelector(".no-customers_createCustomer_PBCPa");
if (boton) {
    boton.addEventListener("click", function () {
        openGeneralInfoCreateCliente();
        changeURLCreateClient("general");
        // Quitar clase activa a todos
        let elementosliDiv = document.querySelectorAll(
            ".lateral-menu-info-clients li div",
        );
        elementosliDiv.forEach((div) => {
            div.classList.remove("index_menuItemActive_m3kE7");
        });
        const divActivo = document.querySelector(
            '[data-testid="client_info_general"]',
        );
        // Añadir clase activa al seleccionado
        divActivo.classList.add("index_menuItemActive_m3kE7");
        // initializeMap();
    });
}

//abre primera vista crear cliente
function openGeneralInfoCreateCliente() {
    const divs = [
    ...
    const openDiv = document.querySelector(".add_client_001");
    const openGeneralInfo = document.querySelector(".client_info_general");
    openDiv.style.display = "block";
    openGeneralInfo.style.display = "block";
}
...
/**
 * MARCAR CLIENTE CONFIANZA
 */
function marcarClienteConfi(contenedorRecibido, seleccionRecibida) {
    // Validar que horaSeleccionada tenga un valor válido
    if (!seleccionRecibida || seleccionRecibida.trim() === "") {
        console.warn("horaSeleccionada está vacía o no es válida");
        return;
    }

    let ContenedorEncontrado = document.querySelector(contenedorRecibido);
    ...
    // Eliminar selección previa
    $(contenedorRecibido + " .index_--selected_oUDGp").removeClass(
        "index_--selected_oUDGp",
    );
    $(contenedorRecibido + " .index_--highlighted__3J43").removeClass(
        "index_--highlighted__3J43",
    );

    // Buscar el elemento con la hora seleccionada
    let seleccionEncontrada = ContenedorEncontrado.querySelector(
        `[data-option="${seleccionRecibida.trim()}"]`,
    );

    if (seleccionEncontrada) {
        $(seleccionEncontrada).addClass(
            "index_--selected_oUDGp index_--highlighted__3J43",
        );
        seleccionEncontrada.style.display = "flex"; // Asegurarse de que el elemento esté visible
    } else {
        // console.warn('No se encontró elemento con data-time=' + seleccionRecibida);
    }
}

//abre cierra desplegable cliente confianza
const contenedor = document.getElementById("conteneforClienteConfianza");
contenedor.addEventListener("click", function (e) {
    // Buscar el div que tiene data-option
    const opcion = e.target.closest("[data-option]");

    if (opcion) {
        const valor = opcion.getAttribute("data-option");

        marcarClienteConfi("#conteneforClienteConfianza", valor);
        document.getElementById("uid-152-input").value = valor;
        $(contenedor).slideToggle();
    }
});

//formatear porcentaje descuento
function formatearPorcentaje(input) {
    ...
    valor = Math.min(100, Math.max(0, parseInt(valor)));

    input.value = valor + "%";
}

/**
 * PESTAÑAS LATERALES INFO CLIENTE
 */
document.querySelectorAll(".lateral-menu-info-clients li").forEach((li) => {
    li.onclick = function () {
        console.log("clic en pestaña lateral info cliente");

        const divDentroDeLi = li.querySelector("div");

        // ⚠️ Si ya está activa, no hacer nada
        if (divDentroDeLi.classList.contains("index_menuItemActive_m3kE7")) {
            return;
        }
        ...
        // Añadir clase activa al seleccionado
        divDentroDeLi.classList.add("index_menuItemActive_m3kE7");

        // Obtener data-testid
        const dataTestId = divDentroDeLi?.getAttribute("data-testid");
        if (!dataTestId) return;

        // 👉 MAPEAR a nombre de tab limpio
        let tab = "";
        switch (dataTestId) {
            case "client_info_general":
                tab = "general";
                break;
            case "client_info_additional":
                tab = "additional";
                break;
            case "client_info_privacy":
                tab = "privacy";
                break;
        }
        ...
        if (tab) {
            changeURLCreateClient(tab);
        }
        console.log(baseCreateUrl);

        // Mostrar contenido
        showDivClientInfo(dataTestId);
    };
});

//cambia la url para crear cliente info, general ...
function changeURLCreateClient(tab) {
...
//al reacargar la página deja solo la original y limpia
document.addEventListener("DOMContentLoaded", function () {
    const url = new URL(window.location.href);

    // Si hay parámetros (tab, create, etc.)
...
//MANEJA MODAL SELECCIONAR IMAGEN NUEVO CLIENTE
...
let cropper;
let currentUrlCropper; // global para controlar la URL anterior

// 👉 Abrir selector al hacer click
if (preview) {
    preview.addEventListener("click", () => {
        input.value = ""; // 🔥 limpia selección anterior
        input.click();
    });
}
...
if (input) {
    input.addEventListener("change", function () {
        ...
        if (currentUrlCropper) {
            URL.revokeObjectURL(currentUrlCropper);
        }

        currentUrlCropper = URL.createObjectURL(file);
        image.src = currentUrlCropper;
...
// — Botón “Añadir” del selector de imagen
if (cropBtn) {
...
// cerrar modal añadir imagen nuevo usuario
