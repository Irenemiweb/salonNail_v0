...
function obtenerAnidadidoReservaFinalizada(data = []) {
    ...
    const soloCreadasYModificadas = data.every(item =>
        item.accion === "creada" || item.accion === "modificada"
    );
...
function  htmlTablaModificacion(item, accionUtilizar, cambiosAgrupados, grupo, serviciosAgrupados, operacionesAgrupadas){
...
function htmlServiciosHistorialModificacion(
    ...
    let itemIdReserva = itemServicio.id_reserva ? itemServicio.id_reserva: itemServicio.reserva_id;
    let cajaRoja = '';
    let cajaVerde = '';

    // 🔴 SERVICIO ELIMINADO
    const esEliminado = Array.isArray(idsReservasEliminadas) &&
    ...
    // 🟢 SERVICIO AÑADIDO
    const esAniadido = Array.isArray(serviciosAniadidos) &&
...
function abrirModalCambios(cambios, accionUtilizar, item, cambiosAgrupados, grupo, serviciosAgrupados, operacionesAgrupadas){
    //esReservaCreada
    ...
    getServiciosByReservas(ids_reservasArray, function (serviciosRecibidos, reservasRecibidas){
        console.log(serviciosRecibidos, "servicios recibidos MODAL");
        console.log(reservasRecibidas, "reservas recibidos MODAL");

        if(esReservaCreada && !hayEliminada && !servicioAniadido.length && !servicio_Aniadido2){
            ...
            serviciosAgrupados.forEach(itemServicio => {
                servicioshtml += htmlServiciosHistorialModificacion(itemServicio);
            });
            html+=servicioshtml;
            ...
            let ids_reservas_eliminadasMostrarHtml = ids_reservas_eliminadas.join(', ');//muestra los ids separados por comas
            cambios.antes={};
            ...
            let idsAccionElimindaGrupo = obtenerIdsAccionEliminadasGrupo(grupo);
            console.log(idsAccionElimindaGrupo, "ids con accion = eliminadas -----hay eliminado" );


            let reservasRecibidasConAnccion = marcarAccionReservas(reservasRecibidas, idsAccionElimindaGrupo);
            console.log(reservasRecibidasConAnccion, "reserasRecibidas con accion-----hay eliminado");

            idsEliminados = obtenerIdsReservasEliminadas(reservasRecibidasConAnccion, idsReservasActivas);
            console.log(idsEliminados, "idsEliminados");
            console.log(idsReservasActivas, "ids reservas activas");

            serviciosFiltradosSinEliminados = filtrarServicios(serviciosRecibidos, idsEliminados);

            serviciosFiltradosSinEliminados.forEach(itemServicio => {
                servicioshtml += htmlServiciosHistorialModificacion(itemServicio, ids_reservas_eliminadas);
            });
            html+=servicioshtml;
            serviciosDespues = obtenerServiciosNoEliminados(serviciosFiltradosSinEliminados, ids_reservas_eliminadas);
            // console.log(serviciosDespues, "servicios despues no eliminados");

            serviciosDespues.forEach(itemServicio => {
                servicioDespuesHtml += htmlServiciosHistorialModificacion(itemServicio);
            });

        }else if (!esReservaCreada && !hayEliminada && servicioAniadido.length) {
            ...
            idsEliminados = obtenerIdsReservasEliminadas(reservasRecibidas, idsReservasActivas);
            serviciosFiltrados = filtrarServicios(serviciosRecibidos, idsEliminados);
            ...
            let esReservaFinalizada = esReservaFinalizadaFuncion(grupo);
            if(esReservaFinalizada){
                let reservaCreada = obtenerAnidadidoReservaFinalizada(grupo);
            ...
            console.log(reservasRecibidasConAnccion, "RESERVAS RECIBIDAS ACCTION AÑADIDO 3");
            idsEliminados = obtenerIdsReservasEliminadas(reservasRecibidasConAnccion, idsReservasActivas);
            ...
            console.log(reservasRecibidas, "reservas recibidas cambiofecha");
            html += `<strong class="text-black">SERVICIOS:</strong>`;
            serviciosAgrupados.forEach(itemServicio => {
                servicioshtml += htmlServiciosHistorialModificacion(
                    itemServicio,
        ...
        if(cambios.despues){
            ...
            Object.keys(cambios.despues).forEach(campo => {
                html += htmlCambiosHistorial(cambios, accionUtilizar, campo, grupo);
            });
        }
         // servicios despues
...
function obtenerServiciosNoEliminados(data, idsReservasEliminadas) {
    return data.filter(item => !idsReservasEliminadas.includes(item.reserva_id));
}
...
function comprobarSiEsCrearReserva(modificacionesAgrupadas){
    let esReservaCreada = Object.values(modificacionesAgrupadas)[0].every(item1 =>
        item1.accion === "creada" &&
...
function seHaAnadidoServicio(data = []) {
    ...
    const hayModificadaValida = data.some(item =>
        item.accion === "modificada" &&
    ...
    return data.filter(item => {
        ...
        return (
            ...
            (typeof antes === "object" && antes !== null && Object.keys(antes).length === 0)
        );
    });
}
//-------------------------------------------------------------------
...
function filtrarServicios(servicios = [], idsEliminados = []) {
    if (!Array.isArray(servicios) || !Array.isArray(idsEliminados)) return servicios;
...
function renderHistorialTimeline(historial){
    ...
    Object.values(operacionesAgrupadas).forEach(grupo => {
        ...
        //pintamos la tabla
        htmlTablaModificacion(item, accionUtilizar ,cambiosAgrupados, grupo, serviciosAgrupados, operacionesAgrupadas);
...
function htmlCambiosHistorial(cambios, accion, campo, grupo) {
    ...
    if (esReservaCreada && !servicio_Aniadido2 && campo !== 'aniadido') {
        // console.log("es reserva creada");
         ...
         if (accion === 'creada' && !['date_time','empleada_id', 'ids_reservas'].includes(campo)) {
            return '';
        }
        ...
        if(campo === 'delete' ){
            ...
            `;
        }else if(esReservaCreada && servicio_Aniadido2 && campo === 'aniadido'){
            despues = cambios.despues[campo];
            despues = formatearValor(campo, despues);
