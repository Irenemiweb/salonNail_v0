...
var mSend= document.querySelector('.m-send');
if(mSend){
    mSend.addEventListener('input', function() {
        // Verificar si hay un salto de línea
        var textarea = this;
  ...
  if (messenger != 0) {
    ...
    $.ajax({
      ...
      success: (data) => {
        ...
        $(".m-header-messaging .user-name").text(data.fetch.name+ 'hola esta aqui');
        // Star status
        data.favorite > 0
        ...
        cancelAttachment();
        messageInput.focus();
         if(currentUser.id_admin === 1){
            cambiarURL('admin/dashboard/Mensajes_administrator');
        }else{
            cambiarURL('panel/Mensajes');
        }
      },
      error: () => {
...
activeStatusChannel.bind("client-closedConversation", function(data){
    if(data.to == auth_id){
        $("body").find(`.messages`).html("");
        $(".messages").find(".message-hint").show();
    }
...
function cerrarConversacion(){
    $('.messenger-sendCard').hide();
    $("body").find(`.messages`).html("");
    $(".messages").append('<p class="message-hint center-el"><span>Please select a chat to start messaging</span></p>');
    $('meta[name="id"]').attr('content', '0');
    $(".m-header-messaging .user-name").text('Afri Nail art studio');
    $(".messenger-infoView .info-name").text('Afri Nail art studio');
    document.getElementsByClassName('header-avatar')[0].style.backgroundImage = '';
    document.getElementsByClassName('add-to-favorite')[0].style.display = 'none';
    $(".messenger-infoView-btns .delete-conversation").css('display', 'none');
    $(".messenger-infoView-shared").css('display', 'none');
    $(".messenger-infoView").find(".avatar").css("background-image", '');
    $(".add-to-favorite").removeClass("favorite");
    $(document).find(".messenger-list-item").removeClass("m-list-active");
}
...
/**
 *-------------------------------------------------------------
 * Trigger closed conversation
 *-------------------------------------------------------------
 */
 function sendCloseConversacionEvent(){
    return clientSendChannel.trigger("client-closedConversation", {
        from: auth_id
      });
}
/**
...
function setContactsLoading(loading = false) {
  if (!loading) {
    $(".listContactChat").find(".loading-contacts").remove();
  } else {
    $(".listContactChat").append(
...
function getContacts() {
    ...
    const listOfContactsDivs = document.querySelectorAll('.listOfContacts');
  if (!contactsLoading && !noMoreContacts) {
    listOfContactsDivs.forEach(div => {

        if (div.classList.contains('listOfContactNotChat') ||
            div.classList.contains('listOfContactNotChatTab')) {
...
function star(user_id) {
  if (getMessengerId() != auth_id) {
    $.ajax({
      ...
      success: (data) => {
        data.status > 0
        ? ($(".add-to-favorite").addClass("favorite"), getFavoritesList())
          : ($(".add-to-favorite").removeClass("favorite"),getFavoritesList());
      },
      error: () => {
...
function getSharedPhotos(user_id) {
    console.log(user_id, url, csrfToken);
...
$(document).ready(function () {
    // if (window.location.href.includes('Mensajes_')) {
        // get contacts list
        getContacts();
  ...
  // click action for favorite button muestra chat cuando click en favoritos
