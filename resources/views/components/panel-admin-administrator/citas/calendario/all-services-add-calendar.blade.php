...
{{-- <div style="border-radius: 12px 0px 0px;" class="offcanvas offcanvas-end" id="offcanvasShowAllServicesChangeAddCalendar" tabindex="-1" aria-labelledby="offcanvasShowAllServicesChangeAddLabelCalendar" data-bs-backdrop="static"> --}}
 @php
        function lightenColor2($rgb, $percentage) {
    ...
    <div class="service-select_container_SSVpA drawer_body_mcPYG">
        <header class="margin-bottom-16 b-custom-header_header_oZL1I">
            <div class="b-custom-header_left b-custom-header_hasIcon_uCjMI b-custom-header_hasIconEllipsis_gYB6P b-custom-header_left_bWmRf">
                <span onclick="backNewReservChangeInitial();" class="exitShowAllServicesChangeModifyInitial b-icon iconFont icon-nav-arrow-left b-custom-header_icon_XtAgm" style="font-size: 20px;"></span>
                <div class="">
        ...
        <div class=" mb-4 pe-3">
            <?php
                $id_inputSearchClient = 'allsevices_add_calendar';
                $onkeyup="" ;
            ?>
        ...
        <div class="scrollable service-select_servicesList_wiG0Q">
            <div>
                ...
                <div>
                    <div class="services-list_header_zR0q6Add_calendar {{ preg_replace('/[^A-Za-z0-9]/', '', str_replace(' ', '', iconv('UTF-8', 'ASCII//TRANSLIT', $categoria->categoria))) }}"
                         data-target="content-{{ $categoria->categoria }}-{{ $index }}Add_calendar">
                        <p class="txt--ellipsis mb-0">{{ $categoria->categoria }}</p>
                        <span class="b-icon iconFont icon-arrow-down" style="font-size: 16px;"></span>
                    </div>
                    <div id="content-{{ $categoria->categoria }}-{{ $index }}Add_calendar" class="serviceListContent {{ preg_replace('/[^A-Za-z0-9]/', '', str_replace(' ', '', iconv('UTF-8', 'ASCII//TRANSLIT', $categoria->categoria))) }}">
                        @foreach ($servicios->where('categoria', $categoria->categoria) as $servicio)
                        @php
                            $backgroundColor = lightenColor2($servicio->borderColor, 70); // Aclara el color un 20%
                        @endphp
                        <div data-servicioChange="{{ $servicio->id }}" class="services-list_serviceVariant_i9qZrAdd_calendar" style="background-color: {{ $backgroundColor }};">
